...
FROM runtime-base AS telebirr-assignment-broker

ARG VCS_REF=unknown
LABEL org.opencontainers.image.title="fetanagent-telebirr-assignment-broker" \
      org.opencontainers.image.revision="${VCS_REF}"
...
# This image contains the database-ingress-only trusted TeleBirr process. Runtime credentials,
# CA material, and pinned public keys are mounted later through the explicit opt-in Compose profile.
FROM runtime-base AS trusted-telebirr-verifier

ARG VCS_REF=unknown
LABEL org.opencontainers.image.title="fetanagent-trusted-telebirr-verifier" \
      org.opencontainers.image.revision="${VCS_REF}"
...
RUN test -n "${FETANAGENT_CHROMIUM_PACKAGE_VERSION}" \
  ...
  && apt-get install --yes --no-install-recommends \
    "chromium=${FETANAGENT_CHROMIUM_PACKAGE_VERSION}" \
    "chromium-common=${FETANAGENT_CHROMIUM_PACKAGE_VERSION}" \
    fonts-liberation \
