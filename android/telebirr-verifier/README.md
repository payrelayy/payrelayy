...
Version `0.5.6` retains the fixed-host, redirect-rejecting, hostname-verified HTTPS origin binding
and adds bounded parsing for the official receipt's current one-cell/bilingual transaction labels,
Unicode spacing, successful status, transfer reason, and year-first timestamp variants. Unattested
or ambiguous documents still fail closed, and the evidence-only build retains no settlement,
execution, or financial authority.
...
versions. Version `0.5.6-secure-provisioning-inert` remains compiled with
