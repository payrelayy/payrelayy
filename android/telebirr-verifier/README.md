...
Version `0.5.7` retains the fixed-host, redirect-rejecting, hostname-verified HTTPS origin binding
and adds the provider's public `Receipt No.` / payment-reference vocabulary, tag-independent bounded
text-node pairing, and its documented `APP` channel variant to the current one-cell/bilingual parser.
Unattested or ambiguous documents still fail closed, and the evidence-only build retains no
...
versions. Version `0.5.7-secure-provisioning-inert` remains compiled with
