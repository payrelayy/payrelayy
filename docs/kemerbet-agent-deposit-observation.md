...
The checked-in executor remains unable to perform a live transfer. This observation supports a
later supervised adapter only after it has all of these controls:
...
- no retry until reconciliation proves that no matching external transfer occurred; and
...
The following cases must be implemented with a deterministic fake transport before any executor
rollout:
...
- delayed, missing, duplicated, or non-`Approved` history records; and
- reconciliation after a success dialog is lost.

These fault cases must not be intentionally reproduced with real money. Any later positive live
test still requires a separate bounded authorization plus the durable execution attempt, incident
stop, serialization, and reconciliation controls. Withdrawal automation remains a later phase
after the deposit workflow is complete.
