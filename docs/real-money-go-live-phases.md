...
| Trusted TeleBirr backend foundation               | **COMPLETE, unprovisioned**        | Migrations are applied and source includes the authenticated database-free bridge, both private Unix-socket brokers, guarded lifecycles/images, isolated no-port Compose stack, and exact HTTPS gateway routes; secrets, runtime LOGIN, DNS, deployment, and Android enrollment remain unprovisioned                                                                                                                                                                                                   |
...
| TeleBirr receiver/profile/signer/device           | **NEXT — VERIFY/PROVISION**        | The source deployment contract now isolates both broker roles and database-free ingress with four exact HTTPS routes and no calendar stop; Android has inert-by-default pinned public trust, encrypted exact-request pairing, pairing-only heartbeat mode, and evidence composition; the authenticated idempotent Owner package issuer is implemented but unconfigured; deploy its migration, then provision the profile, child key, signers, broker credentials, DNS, signed APK, and dedicated phone |
...
7. The Android source now implements build-pinned public trust, encrypted one-use pairing and exact
   ...
   and no browser persistence of package material. Deploy its migration and configure only the
   reviewed public assignment-signer ID; then finish the signed operational APK and physical-device
   refusal matrix before treating this item as complete.
