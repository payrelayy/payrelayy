...
| Windows companion v0.1.5 release                 | **IMPLEMENTED — PACKAGE NEXT**      | The release adds the signed exact-five worker, DPAPI crash ledger, strict fixed-path request gate, stale-result/assignment binding, and updated no-money launcher text; publish the exact tagged GitHub Actions package after this commit passes every required check                                                                                                                                                                                                                                                              |
| Local KemerBet identity and device pairing       | **COMPLETE — LOCAL, NO-MONEY**      | The exact `/agents` identity boundary, absent visible login/CAPTCHA state, stable reviewed account header, DPAPI CurrentUser identity/key protection, and server-signed enrollment certificate are verified on the dedicated Windows profile; the v0.1.5 package reuses this protected pairing                                                                                                                                                                                                                                     |
| Signed exact-five companion lookup               | **IMPLEMENTED — DEPLOY/PROVE NEXT** | Owner issuance/status, exact-five immutable member snapshot, one-use HTTP replay ledger, signed bridge poll/result routes, local sequential Find-only execution, signed redacted result acceptance, and no-calendar-expiry runtime deployment are wired; apply the migration, deploy the v2 no-money bridge and Owner gateway, install v0.1.5, then capture one live redacted completion proof                                                                                                                                     |
