...
| TeleBirr receiver/profile/signer/device           | **NEXT — VERIFY/PROVISION**        | Pairing/command/ack codecs, private lease/open/sign broker, durable device-state PostgreSQL adapter, and both fixed Unix-socket adapters exist in source; finish their lifecycles, verify the Owner-saved receiver, then provision the exact profile, scoped opening key, signer, bridge origin, and dedicated phone |
...
credential. The source server requires a non-root-owned mode-`0700` directory and creates one
inode-checked mode-`0600` socket with no TCP listener. No runtime credential, executable lifecycle,
container, public ingress, pilot switch, settlement, or money capability is enabled by that source
increment.
...
6. The canonical fixed assignment and device-state Unix-socket servers and bridge adapters exist in
   source. Finish the device-state config/application/container lifecycle, then deploy both private
   brokers and compose the bridge as separate processes/containers; do not add a public broker
   listener, database credential to the bridge, or exposed protected-reference opening operation.
