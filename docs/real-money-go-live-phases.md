...
5. The reviewed migrations, strict secret/config loader, singleton direct verify-full PostgreSQL
   composition, exact two-function catalog preflight, fail-closed application lifecycle, and
   no-port image target exist. Next, generate the bounded dedicated runtime `LOGIN` and provision
   only the guarded files outside Git.
6. The canonical fixed Unix-socket server and bridge adapter exist in source. Deploy the broker and
   compose the bridge as separate processes/containers; do not add a public broker listener or
   expose the protected-reference opening operation.
