...
| TeleBirr receiver/profile/signer/device           | **NEXT — VERIFY/PROVISION**        | The source deployment contract now isolates both broker roles and database-free ingress with four exact HTTPS routes and no calendar stop; verify the Owner-saved receiver, then provision the profile, child key, signers, broker credentials, exact origin, DNS, and dedicated phone               |
