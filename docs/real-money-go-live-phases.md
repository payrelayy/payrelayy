...
The bot now puts **Deposit with TeleBirr** first in `/menu`. The same stateless Force Reply prompt is
available through `/deposit`; the customer sends the Player ID on the first line and the TeleBirr
transaction number, receipt URL, or SMS after it. It extracts one bounded candidate locally,
preserves the existing proof/tracking identity, and requires explicit resubmission when multiple IDs are found.
...
`apps/bot/src/index.ts` wires `/start`, `/menu`, Player-ID actions, the button-first TeleBirr prompt,
the legacy amount-free `/deposit <provider> <Player ID> <transaction reference>` compatibility
command, and `/deposit_status`. The one-reply prompt keeps no bot-local draft: every required value
arrives in the same Telegram update and reaches the existing dry-run-only API capture. Its status
operation remains separate from the legacy deposit-intent lookup. Preserve that explicit
distinction when implementing live proof lineage, saved Player-ID buttons, and durable completion
notifications. Do not promote old simulation records into live payments.
...
| Trusted TeleBirr backend foundation              | **COMPLETE, trust provisioned**     | Migrations are applied; the immutable staging assignment signer is active/unrevoked; protected independent assignment/bridge keys, scoped reference-opening material, runtime credential candidates, and bridge manifest are installed in the protected GitHub staging environment; the authenticated database-free bridge, both private Unix-socket brokers, guarded images, no-port Compose stack, and exact HTTPS routes are merged; bounded runtime LOGIN, three-service deployment, DNS, and Android enrollment remain closed |
...
| Windows companion v0.1.10 release                | **IMPLEMENTED — PACKAGE NEXT**      | The release adds twelve-hour human-operated pairing and exact-five windows while retaining just-in-time one-use claim requests, the signed exact-five worker, DPAPI crash ledger, strict fixed-path request gate, stale-result/assignment binding, and updated no-money launcher text; publish the exact tagged GitHub Actions package after this commit passes every required check                                                                                                                                               |
| Local KemerBet identity and device pairing       | **COMPLETE — LOCAL, NO-MONEY**      | The exact `/agents` identity boundary, absent visible login/CAPTCHA state, stable reviewed account header, DPAPI CurrentUser identity/key protection, and server-signed enrollment certificate are verified on the dedicated Windows profile; the v0.1.10 package reuses this protected pairing                                                                                                                                                                                                                                    |
| Signed exact-five companion lookup               | **IMPLEMENTED — DEPLOY/PROVE NEXT** | Owner issuance/status, twelve-hour exact-five immutable member snapshot, one-use HTTP replay ledger, signed bridge poll/result routes, local sequential Find-only execution, signed redacted result acceptance, and no-calendar-expiry runtime deployment are wired; apply the migration, deploy the v2 no-money bridge and Owner gateway, install v0.1.10, then capture one live redacted completion proof                                                                                                                        |
| TeleBirr receiver/profile/signer/device          | **NEXT — DEPLOY/PAIR**              | The authenticated Owner package issuer migration is applied, its reviewed signer ID is merged into the staging Owner composition, and the signed one-signer/v2/fingerprint-verified Android 0.5.4 pairing-only prerelease is published; deploy the latest Owner composition, provision the bounded broker roles/files and three-service stack, publish DNS only after healthy negative smokes, then install and pair the dedicated phone; evidence mode and every money gate remain disabled                                       |
...
3. The independent P-256 assignment and bridge signers are generated in the protected GitHub
   staging environment, and the exact immutable assignment signer is active and unrevoked in
   PostgreSQL. Keep both private keys out of Android, source, logs, and the public bridge.
4. The TeleBirr/reference-opening scoped child-key document is generated and protected in the
   staging environment. Provision only that document to the assignment broker during the guarded
   three-service deployment. Never provision the encryption or fingerprint master to the broker.
...
6. The canonical fixed assignment and device-state Unix-socket servers, bridge adapters, all three
   ...
   exposed protected-reference opening operation. Follow the exact file, network, gateway, DNS,
   smoke, and rollback order in
   [`../infra/telebirr-device-pilot.md`](../infra/telebirr-device-pilot.md).
7. The Android source now implements build-pinned public trust, encrypted one-use pairing and exact
   request replay, pairing-only heartbeat, evidence-mode lease/official observation/signature/upload,
   bounded foreground operation, expiry, and signed revocation responses. The authenticated Owner
...
4. Review the aggregate projection: five Players, 25 ETB per transaction/Player, 125 ETB aggregate,
   five reservations, twelve-hour window.
...
- [`../infra/telebirr-device-pilot.md`](../infra/telebirr-device-pilot.md) — no-money three-service
  Compose isolation, guarded files, exact gateway/DNS sequence, health evidence, and rollback.
