#!/usr/bin/env bash
# One-use, root-console-only H21 -> H22 guard correction. It preserves the
# terminal H19 candidate-gateway receipt, replaces only the ingress guard, and
# corrects the deterministic ordering check for the two terminal receipt files.
# It does not run Compose, alter a database, change a container, or move money.
set -euo pipefail

readonly SAFE_PATH='/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin'
readonly INGRESS_GUARD='/usr/local/sbin/fetanagent-production-ingress-h19'
readonly GUARD_INSTALLING='/usr/local/sbin/.fetanagent-production-ingress-h19.h22-installing'
readonly DEPLOY_SUDOERS='/etc/sudoers.d/fetanagent-staging-deploy-helper'
readonly DEPLOY_SUDOERS_DISABLED='/etc/sudoers.d/.fetanagent-staging-deploy-helper.h22-disabled'
...
readonly H21_RELEASE='ac0df375fb2ef6257d30c39f6cb4c2fa1dab01e2'
readonly H21_INTENT_SHA256='e28e67e611ca8ece8cdddaa9c98634c0fb93324d547d50aa771920c89e59cc18'
readonly H21_COMPLETION_SHA256='15e1c3e5860aa6355f85be4114a5146f20e2683b1eb5da9b8351baf7a6b6d9ea'
readonly H20_GUARD_SHA256='4481190534fb41f057f0f3c716d74ba1f6445da009d491936c1098bdaa756f5a'
readonly H21_GUARD_SHA256='a4e31a95bfb4826634069cdc31f745ed53cccb0db2cc01578851ac8330623813'
...
readonly TRANSITION_INTENT_SHA256='b0dd0ff0f66d961448e6e214feea8806627bf9f5aac1995436b2105f3fce6537'
readonly REVIEWED_TERMINAL_RECEIPT_SHA256='d33d2e51852fabdfd8f750bfd16118fe987e8b4201483b1cc2f688063047d559'
readonly REVIEWED_SUCCESSOR_GUARD_SHA256='0b4a9b31a893073e725bfc97fc6ef3f6589fd9b5d720da5003e987ad0dcc7f17'
...
readonly SCRIPT_BASENAME='fetanagent-h19-terminal-receipt-order-guard-bridge-v22.sh'
readonly CONFIRMATION='I-UNDERSTAND-THIS-CORRECTS-H19-TERMINAL-RECEIPT-ORDER-WITH-NO-PRODUCTION-OR-MONEY-MUTATION'

export PATH="$SAFE_PATH"
umask 077

die() {
  printf 'FetanAgent H22 terminal-receipt guard bridge failed closed: %s\n' "$1" >&2
  exit 1
}

[[ $# -eq 5 ]] ||
  die 'expected the H22 release, guard digest, terminal-receipt digest, candidate image ID, and exact confirmation'
readonly BRIDGE_RELEASE="$1"
readonly SUCCESSOR_GUARD_SHA256="$2"
readonly TERMINAL_RECEIPT_SHA256="$3"
readonly CANDIDATE_IMAGE_ID="$4"
readonly PROVIDED_CONFIRMATION="$5"
readonly STAGING_ROOT="/root/fetanagent-h19-terminal-receipt-order-guard-bridge-v22-$BRIDGE_RELEASE"
...
readonly H22_ROOT="$H22_PARENT/$BRIDGE_RELEASE"
readonly H22_INSTALLING="$H22_PARENT/.installing-$BRIDGE_RELEASE"
readonly H21_ROOT="$H21_PARENT/$H21_RELEASE"
readonly TRANSITION_ROOT="$TRANSITION_PARENT/$H19_RELEASE"

[[ "$BRIDGE_RELEASE" =~ ^[0-9a-f]{40}$ &&
  "$BRIDGE_RELEASE" != "$PROTECTED_RELEASE" && "$BRIDGE_RELEASE" != "$H19_RELEASE" &&
  "$BRIDGE_RELEASE" != "$H21_RELEASE" ]] ||
  die 'H22 requires one distinct full release SHA for correction provenance'
[[ "$SUCCESSOR_GUARD_SHA256" == "$REVIEWED_SUCCESSOR_GUARD_SHA256" &&
  ...
  die 'the staged H22 values do not match the reviewed terminal recovery chain'
[[ "$SUCCESSOR_GUARD_SHA256" != "$H21_GUARD_SHA256" ]] ||
  die 'the successor guard must be distinct from H21'
[[ "$PROVIDED_CONFIRMATION" == "$CONFIRMATION" ]] ||
  die 'the exact one-use no-money H22 confirmation is required'
...
require_h21_record_and_artifacts() {
  require_exact_directory "$H21_PARENT" "$H21_RELEASE" || return 1
  require_exact_directory "$H21_ROOT" completed-v1 intent-v1 interrupted-transition-intent \
    predecessor-ingress-guard || return 1
  require_exact_file "$H21_ROOT/intent-v1" "$H21_INTENT_SHA256" 600 4096 || return 1
  require_exact_file "$H21_ROOT/completed-v1" "$H21_COMPLETION_SHA256" 600 4096 || return 1
  require_exact_file "$H21_ROOT/predecessor-ingress-guard" "$H20_GUARD_SHA256" 400 || return 1
  require_exact_file "$H21_ROOT/interrupted-transition-intent" \
    "$TRANSITION_INTENT_SHA256" 400 8192 || return 1
  require_exact_file /usr/local/sbin/fetanagent-staging-deploy-helper \
    "$H20_HELPER_SHA256" 755 || return 1
  ...
  require_exact_file /etc/sudoers.d/fetanagent-staging-continuous-availability \
    "$H20_SUDOERS_SHA256" 440 65536
}

require_terminal_transition() {
  require_exact_directory "$TRANSITION_PARENT" "$H19_RELEASE" || return 1
  require_exact_directory "$TRANSITION_ROOT" completed-v1 intent-v1 || return 1
  require_exact_file "$TRANSITION_ROOT/intent-v1" "$TRANSITION_INTENT_SHA256" 600 8192 || return 1
  require_exact_file "$TRANSITION_ROOT/completed-v1" "$TERMINAL_RECEIPT_SHA256" 600 8192
}

guard_state() {
  if require_exact_file "$INGRESS_GUARD" "$H21_GUARD_SHA256" 755 2>/dev/null; then
    printf old
...
namespace_state() {
  if [[ ! -e "$H22_PARENT" && ! -L "$H22_PARENT" ]]; then
    printf absent
    return
  fi
  [[ ! -L "$H22_PARENT" && -d "$H22_PARENT" &&
    "$(realpath -- "$H22_PARENT")" == "$H22_PARENT" &&
    "$(stat --format='%U:%G:%a' "$H22_PARENT")" == 'root:root:700' ]] || return 1
  local entries
  entries="$(find -P "$H22_PARENT" -mindepth 1 -maxdepth 1 -printf '%f:%y\n' |
...
require_candidate_boundaries() {
  ...
  [[ "$(gateway_image_id)" == "$CANDIDATE_IMAGE_ID" &&
    "$(gateway_caddy_sha256)" == "$CANDIDATE_CADDY_SHA256" &&
    "$(production_boundary_digest)" == "$POST_PRODUCTION_BOUNDARY_SHA256" &&
    "$(canonical_nine_digest)" == "$CANONICAL_NINE_SHA256" &&
    "$(shared_ingress_digest)" == "$POST_SHARED_INGRESS_SHA256" &&
...
expected_intent() {
  printf '%s\n' \
    ...
    "h21_bridge_release=$H21_RELEASE" \
    "h21_bridge_intent_sha256=$H21_INTENT_SHA256" \
    "h21_bridge_completion_sha256=$H21_COMPLETION_SHA256" \
    "h19_bridge_release=$H19_RELEASE" \
    "candidate_gateway_release=$H19_RELEASE" \
    "predecessor_ingress_guard_sha256=$H21_GUARD_SHA256" \
    "successor_ingress_guard_sha256=$SUCCESSOR_GUARD_SHA256" \
    "transition_intent_sha256=$TRANSITION_INTENT_SHA256" \
    "terminal_transition_receipt_sha256=$TERMINAL_RECEIPT_SHA256" \
    "post_production_boundary_sha256=$POST_PRODUCTION_BOUNDARY_SHA256" \
    "post_shared_ingress_sha256=$POST_SHARED_INGRESS_SHA256" \
    "post_tls_leaf_sha256=$TLS_LEAF_SHA256" \
...
expected_completion() {
  ...
  expected_intent | awk 'NR == 2 {$0="state=terminal-receipt-order-guard-installed"} {print}'
...
archive_file() {
  local source="$1" target="$2" digest="$3" maximum="$4" mode="$5" temporary
  temporary="$target.installing"
  if [[ -e "$target" || -L "$target" ]]; then
    [[ ! -e "$temporary" && ! -L "$temporary" ]] || return 1
  ...
  if [[ ! -e "$temporary" && ! -L "$temporary" ]]; then
    require_exact_file "$source" "$digest" "$mode" "$maximum" || return 1
...
require_terminal_record() {
  require_exact_directory "$H22_PARENT" "$BRIDGE_RELEASE" || return 1
  require_exact_directory "$H22_ROOT" completed-v1 intent-v1 predecessor-ingress-guard \
    terminal-transition-receipt || return 1
  require_expected_record "$H22_ROOT/intent-v1" 600 expected_intent || return 1
  require_expected_record "$H22_ROOT/completed-v1" 600 expected_completion || return 1
  require_exact_file "$H22_ROOT/predecessor-ingress-guard" "$H21_GUARD_SHA256" 400 || return 1
  require_exact_file "$H22_ROOT/terminal-transition-receipt" \
    "$TERMINAL_RECEIPT_SHA256" 400 8192
}
...
require_predecessor_record_output() {
  ...
  [[ "${#RECORD_LINES[@]}" -eq 11 && "${RECORD_LINES[0]}" == active &&
    ...
    "${RECORD_LINES[10]}" == "$H21_GUARD_SHA256" ]]
}
...
require_staged_bundle || die 'the root-owned staged H22 bundle is not exact'
require_exact_droplet || die 'the DigitalOcean Droplet identity is not exact'
require_h21_record_and_artifacts || die 'the terminal H21 provenance or installed artifact chain is invalid'
require_terminal_transition || die 'the completed H19 transition evidence is not exact'
require_candidate_boundaries || die 'a protected candidate boundary is not exact'
state="$(namespace_state)" || die 'the H22 namespace is invalid'
guard="$(guard_state)" || die 'the installed guard is neither the H21 predecessor nor H22 successor'
if [[ "$guard" == old ]]; then
  require_predecessor_record_output || die 'the installed H21 guard rejected its provenance chain'
fi
case "$state:$guard" in
  absent:old|empty-parent:old|installing:old|installing:new|completed:new) ;;
  *) die 'the H22 evidence and installed-guard topology is causally invalid' ;;
esac
if [[ "$state" == completed ]]; then
  require_terminal_record || die 'the terminal H22 record is invalid'
fi
...
require_h21_record_and_artifacts || die 'the H21 chain changed after lock acquisition'
require_terminal_transition || die 'the terminal transition changed after lock acquisition'
require_candidate_boundaries || die 'a candidate boundary changed after lock acquisition'
guard="$(guard_state)" || die 'the installed guard changed after lock acquisition'
if [[ "$guard" == old ]]; then
  require_predecessor_record_output || die 'the H21 provenance changed after lock acquisition'
fi
disable_deploy_grant || die 'the staging deployment capability could not be isolated'

state="$(namespace_state)" || die 'the H22 namespace changed unexpectedly'
if [[ "$state" != completed ]]; then
  if [[ "$state" == absent ]]; then
    install -d -o root -g root -m 0700 "$H22_PARENT" ||
      die 'the H22 evidence parent could not be created'
    sync -f "$(dirname -- "$H22_PARENT")"
    state=empty-parent
  fi
  if [[ "$state" == empty-parent ]]; then
    install -d -o root -g root -m 0700 "$H22_INSTALLING" ||
      die 'the H22 installing root could not be created'
    sync -f "$H22_PARENT"
    state=installing
  fi
  [[ "$state" == installing ]] || die 'the H22 installing namespace is invalid'
  publish_record "$H22_INSTALLING" intent-v1 expected_intent ||
    die 'the H22 intent could not be sealed'
  archive_file "$INGRESS_GUARD" "$H22_INSTALLING/predecessor-ingress-guard" \
    "$H21_GUARD_SHA256" 2097152 755 ||
    die 'the H21 ingress guard could not be archived'
  archive_file "$TRANSITION_ROOT/completed-v1" \
    "$H22_INSTALLING/terminal-transition-receipt" "$TERMINAL_RECEIPT_SHA256" 8192 600 ||
    die 'the terminal transition receipt could not be archived'
  install_successor_guard || die 'the H22 successor guard could not be installed atomically'
  publish_record "$H22_INSTALLING" completed-v1 expected_completion ||
    die 'the H22 completion could not be sealed'
  require_exact_directory "$H22_INSTALLING" completed-v1 intent-v1 \
    predecessor-ingress-guard terminal-transition-receipt &&
    require_expected_record "$H22_INSTALLING/intent-v1" 600 expected_intent &&
    require_expected_record "$H22_INSTALLING/completed-v1" 600 expected_completion &&
    require_exact_file "$H22_INSTALLING/predecessor-ingress-guard" \
      "$H21_GUARD_SHA256" 400 &&
    require_exact_file "$H22_INSTALLING/terminal-transition-receipt" \
      "$TERMINAL_RECEIPT_SHA256" 400 8192 ||
    die 'the completed H22 installing record is invalid'
  [[ ! -e "$H22_ROOT" && ! -L "$H22_ROOT" ]] ||
    die 'the terminal H22 root appeared unexpectedly'
  mv -- "$H22_INSTALLING" "$H22_ROOT" || die 'the H22 record could not become terminal'
  sync -f "$H22_PARENT"
fi

require_terminal_record || die 'the terminal H22 record is invalid'
require_exact_file "$INGRESS_GUARD" "$SUCCESSOR_GUARD_SHA256" 755 ||
  die 'the installed H22 successor guard is not exact'
require_h21_record_and_artifacts || die 'the H21 chain changed during H22 installation'
require_terminal_transition || die 'the terminal transition changed during H22 installation'
require_candidate_boundaries || die 'a candidate boundary changed during H22 installation'
env -i PATH="$SAFE_PATH" HOME='/root' "$INGRESS_GUARD" inspect "$H19_RELEASE" ||
  die 'the H22 successor guard rejected the exact candidate state'
require_successor_record_output || die 'the H22 successor record chain is invalid'
restore_deploy_grant || die 'the staging deployment capability could not be restored safely'
require_terminal_record && require_successor_record_output && require_candidate_boundaries ||
  die 'the restored H22 transaction is not exact'
flock --unlock 9 || die 'the shared mutation lock could not be released'
exec 9>&-

printf '%s\n' \
  'FetanAgent H22 terminal-receipt order correction installed or validated: candidate gateway unchanged; terminal evidence preserved; database untouched; money moved=false.'
