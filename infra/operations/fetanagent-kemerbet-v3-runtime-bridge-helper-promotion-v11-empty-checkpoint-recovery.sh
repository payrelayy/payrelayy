#!/usr/bin/env bash
# Root-console-only recovery for the one exact empty pre-intent checkpoint left by
# the reviewed H11 runtime-bridge promotion. This does not erase or normalize any
# partial evidence. It accepts the observed empty checkpoint, its exact monotonic
# append-only interruption prefixes, or an already-complete H11 promotion. It performs
# no lookup, recheck, deposit, transfer, withdrawal, executor action, or financial action.
...
readonly INSTALLING_HELPER_PARTIAL='/usr/local/sbin/.fetanagent-staging-deploy-helper.v3-runtime-bridge-v11-installing.partial'
...
die() {
  printf 'FetanAgent H11 empty-checkpoint recovery failed closed: %s\n' "$1" >&2
  exit 1
}

[[ $# -eq 3 ]] || die 'expected the exact bridge release, reviewed helper digest, and original H11 confirmation'
...
[[ "$BRIDGE_RELEASE" == "$REVIEWED_BRIDGE_RELEASE" ]] ||
  die 'this recovery is pinned to the one reviewed H11 bridge release'
...
[[ "$PROVIDED_CONFIRMATION" == "$CONFIRMATION" ]] || die 'the original exact H11 confirmation is required'
[[ "$(id -u)" == '0' && "$(id -un)" == 'root' ]] ||
  die 'run this recovery only in the DigitalOcean root console'
...
require_no_other_mutator_processes() {
  ...
  for cmdline in /proc/[0-9]*/cmdline; do
    ...
    while IFS= read -r -d '' argument; do
      ...
      case "$basename" in
        "$ORIGINAL_PROMOTION_BASENAME"|"$RECOVERY_BASENAME") return 1 ;;
      esac
    done <"$cmdline" || true
  done
}
...
classify_rotation() {
  env -i PATH="$SAFE_PATH" python3 -I - "$ROTATION_PARENT" "$BRIDGE_RELEASE" <<'PY'
import os
import stat
import sys
...
if entries == [f'.installing-{release}']:
    print('interrupted')
...
require_exact_empty_checkpoint() {
  [[ ! -L "$ROTATION_PARENT" && -d "$ROTATION_PARENT" &&
    ...
    "$(find -P "$ROTATION_PARENT" -mindepth 1 -maxdepth 1 -printf '%f\n')" == \
      ".installing-$BRIDGE_RELEASE" ]] || return 1
...
cleanup() {
  ...
  if [[ "$status" -ne 0 && "$grant_disabled" == 'true' ]]; then
    printf '%s\n' \
      'FetanAgent H11 recovery stopped with the deployment grant disabled. Rerun this exact reviewed recovery; do not delete evidence or restore the grant manually.' >&2
  fi
  exit "$status"
}
...
rotation_state="$(classify_rotation)" ||
  die 'the H11 namespace is not the exact interrupted prefix or completed record'
case "$rotation_state" in
  interrupted)
    require_interrupted_prefix_consistency ||
      die 'the interrupted H11 prefix, helper, installer, or disabled grant is inconsistent'
    grant_disabled='true'
    ;;
  completed)
    require_exact_completed_namespace || die 'the completed H11 namespace is invalid'
    ...
    ;;
  *) die 'the H11 recovery state is impossible' ;;
esac

trap cleanup EXIT
open_lock || die 'the exact staging mutation lock is unsafe or another mutation is active'
require_no_other_mutator_processes || die 'another helper or H11 promotion process is active'
...
[[ "$(classify_rotation)" == "$rotation_state" ]] ||
  die 'the H11 namespace changed before lock acquisition'

if [[ "$rotation_state" == 'completed' ]]; then
  require_exact_completed_namespace || die 'the completed H11 namespace changed under lock'
else
  require_interrupted_prefix_consistency || die 'the interrupted H11 prefix changed under lock'
  if require_helper_file "$TARGET" "$PREDECESSOR_HELPER_SHA256" 755; then
    require_stopped_historical_overlay ||
      die 'the exact stopped historical-overlay boundary is no longer ready'
  else
    require_helper_file "$TARGET" "$SUCCESSOR_HELPER_SHA256" 755 ||
      die 'the interrupted H11 helper state is invalid'
    require_stopped_durable_boundary ||
      die 'the stopped Docker volumes or identity binding changed after helper replacement'
  fi
  require_no_other_mutator_processes ||
    die 'another helper or H11 promotion process appeared during revalidation'
  require_interrupted_prefix_consistency ||
    die 'the interrupted H11 prefix changed after stopped-boundary revalidation'

  publish_record "$ROTATION_INSTALLING" intent-v1 0600 expected_intent ||
    die 'the H11 intent could not be completed atomically'
  require_interrupted_prefix_consistency || die 'the H11 intent publication is inconsistent'
  ...
  if [[ ! -e "$ROTATION_INSTALLING/predecessor-helper" &&
    ...
    copy_root_file_atomically "$TARGET" \
      ...
      die 'the H10 helper archive could not be completed atomically'
  fi
  require_helper_file "$ROTATION_INSTALLING/predecessor-helper" \
    "$PREDECESSOR_HELPER_SHA256" 400 || die 'the archived H10 helper is invalid'
  require_interrupted_prefix_consistency || die 'the H10 helper archive is inconsistent'

  if require_helper_file "$TARGET" "$PREDECESSOR_HELPER_SHA256" 755; then
    if [[ ! -e "$INSTALLING_HELPER" && ! -L "$INSTALLING_HELPER" ]]; then
      copy_root_file_atomically "$STAGED_HELPER" "$INSTALLING_HELPER_PARTIAL" \
        "$INSTALLING_HELPER" 0600 0755 "$SUCCESSOR_HELPER_SHA256" ||
        die 'the H11 helper installer could not be completed atomically'
    fi
    require_helper_file "$INSTALLING_HELPER" "$SUCCESSOR_HELPER_SHA256" 755 ||
      die 'the installing H11 helper is invalid'
    [[ ! -e "$INSTALLING_HELPER_PARTIAL" && ! -L "$INSTALLING_HELPER_PARTIAL" ]] ||
      die 'a partial H11 helper installer remains'
  ...
  [[ ! -e "$INSTALLING_HELPER" && ! -L "$INSTALLING_HELPER" &&
    ! -e "$INSTALLING_HELPER_PARTIAL" && ! -L "$INSTALLING_HELPER_PARTIAL" ]] ||
    die 'an H11 helper installer residue remains after replacement'
  require_interrupted_prefix_consistency || die 'the H11 helper replacement is inconsistent'

  publish_record "$ROTATION_INSTALLING" completed-v1 0600 expected_completion ||
    die 'the H11 completion could not be completed atomically'
  require_exact_rotation_record "$ROTATION_INSTALLING" ||
    die 'the completed installing H11 record is invalid'
  [[ ! -e "$ROTATION_ROOT" && ! -L "$ROTATION_ROOT" ]] ||
  ...
  require_exact_completed_namespace || die 'the final H11 rotation evidence is invalid'
fi
...
require_no_other_mutator_processes || die 'another helper or H11 promotion process remained'
...
[[ "$(classify_rotation)" == 'completed' ]] || die 'the completed H11 namespace disappeared'
require_exact_completed_namespace || die 'the H11 state changed before grant restoration'
if [[ "$grant_disabled" == 'true' ]]; then
...
printf '%s\n' \
  'KemerBet H11 recovery completed or validated: historical R10 overlay preserved; future reviewed releases enabled; Transfer disabled.'
