#!/usr/bin/env bash
# One-use, root-console-only second rotation of an installed KemerBet v3 successor helper.
# The immutable v2-to-v3 evidence and first helper-rotation link remain untouched;
# this operation appends one exact second link, keeps staging stopped, and never
# enables Transfer.
...
readonly PREDECESSOR_ROTATION_PARENT='/var/lib/fetanagent/kemerbet-readiness-v3-helper-rotation'
readonly ROTATION_PARENT='/var/lib/fetanagent/kemerbet-readiness-v3-helper-rotation-v2'
...
readonly PREDECESSOR_SUDOERS_DISABLED='/etc/sudoers.d/.fetanagent-staging-deploy-helper.v3-rotation-disabled'
readonly SUDOERS_DISABLED='/etc/sudoers.d/.fetanagent-staging-deploy-helper.v3-rotation-v2-disabled'
readonly PREDECESSOR_INSTALLING_HELPER='/usr/local/sbin/.fetanagent-staging-deploy-helper.v3-rotation-installing'
readonly PREDECESSOR_INSTALLING_HELPER_PARTIAL="${PREDECESSOR_INSTALLING_HELPER}.partial"
readonly PREDECESSOR_ROLLBACK_HELPER='/usr/local/sbin/.fetanagent-staging-deploy-helper.v3-rotation-rollback'
readonly PREDECESSOR_ROLLBACK_HELPER_PARTIAL="${PREDECESSOR_ROLLBACK_HELPER}.partial"
readonly INSTALLING_HELPER='/usr/local/sbin/.fetanagent-staging-deploy-helper.v3-rotation-v2-installing'
readonly INSTALLING_HELPER_PARTIAL="${INSTALLING_HELPER}.partial"
readonly ROLLBACK_HELPER='/usr/local/sbin/.fetanagent-staging-deploy-helper.v3-rotation-v2-rollback'
...
readonly PREDECESSOR_RELEASE='8fe693b51b5426c3f358bba67519459161a0ebf9'
readonly PREDECESSOR_HELPER_SHA256='f98047953fb9249d7dbcd13be6cf1a145b53a4952a760b36d5ba8bfab2f36f82'
readonly REVIEWED_SUCCESSOR_HELPER_SHA256='05b0f2c8eb68716d20ad4878f1fff96c2f6a22e532e0b9c52a664e153b49e6fe'
readonly CONFIRMATION='I-UNDERSTAND-THIS-APPENDS-SECOND-V3-HELPER-ROTATION-WITH-TRANSFER-DISABLED'
...
die() {
  printf 'FetanAgent v3 second helper rotation failed closed: %s\n' "$1" >&2
  exit 1
}
...
readonly STAGING_ROOT="/root/fetanagent-v3-helper-rotation-v2-$SUCCESSOR_RELEASE"
...
load_exact_predecessor_rotation_evidence() {
  ...
  inspection="$(env -i PATH="$SAFE_PATH" python3 -I - \
    "$PREDECESSOR_ROTATION_PARENT" "$BASE_SUCCESSOR_RELEASE" "$PREDECESSOR_RELEASE" \
    "$BASE_SUCCESSOR_HELPER_SHA256" "$PREDECESSOR_HELPER_SHA256" \
    "$BASE_SUCCESSOR_INTENT_SHA256" "$BASE_SUCCESSOR_COMPLETION_SHA256" \
    "$BASE_BINDING_V2_SHA256" "$BASE_PREDECESSOR_HELPER_SHA256" \
    "$BASE_BINDING_V3_SHA256" <<'PY'
import hashlib
import os
import re
import stat
...
if (
    release.fullmatch(base_release) is None
    or release.fullmatch(predecessor_release) is None
    or predecessor_release == base_release
):
    reject()
exact_directory(parent, [predecessor_release])
root = f'{parent}/{predecessor_release}'
exact_directory(root, ['completed-v1', 'intent-v1', 'predecessor-helper'])
...
if (
    ...
    or intent[0] != 'contract=fetanagent-kemerbet-readiness-v3-helper-rotation-v1'
    or intent[1] != 'state=authorized'
    or intent[2] != f'predecessor_release={base_release}'
    or intent[3] != f'successor_release={predecessor_release}'
    or intent[4] != f'predecessor_helper_sha256={base_helper}'
    ...
    or not intent[11].startswith('compose5_durable_volume_digest=')
    or sha.fullmatch(intent[11].split('=', 1)[1]) is None
    or not intent[12].startswith('compose5_profile_config_hash=')
    or sha.fullmatch(intent[12].split('=', 1)[1]) is None
    or not intent[13].startswith('compose5_session_control_config_hash=')
    or sha.fullmatch(intent[13].split('=', 1)[1]) is None
    or not intent[14].startswith('compose5_volume_version=')
    or compose_version.fullmatch(intent[14].split('=', 1)[1]) is None
    or completion[:1] != intent[:1]
    or completion[1] != 'state=successor-installed'
    or completion[2:15] != intent[2:15]
    or completion[15] !=
       f'rotation_intent_sha256={hashlib.sha256(intent_data).hexdigest()}'
    or intent_data != ('\n'.join(intent) + '\n').encode('ascii')
    or completion_data != ('\n'.join(completion) + '\n').encode('ascii')
    or hashlib.sha256(archived_helper).hexdigest() != base_helper
):
    reject()
...
print(intent[12].split('=', 1)[1])
print(intent[13].split('=', 1)[1])
...
cleanup() {
  ...
  if [[ "$status" -ne 0 && "$sudoers_disabled" == 'true' ]]; then
    ...
    printf '%s\n' \
      'FetanAgent v3 second helper rotation stopped with the deployment grant disabled. Whether the append-only record is pending or final, rerun this exact operation with the same successor release and helper digest from the root console; do not restore the grant manually.' >&2
  fi
  exit "$status"
}
...
require_predecessor_rotation_global_residue_absent ||
  die 'the first helper rotation still has global transaction residue'
...
load_exact_predecessor_rotation_evidence ||
  die 'the exact completed first helper-rotation link is invalid'
require_predecessor_rotation_global_residue_absent ||
  die 'first-rotation residue appeared after lock acquisition'
require_stopped_no_transfer_boundary ||
  die 'rotation requires a stopped, disarmed, no-transient, exact Compose 5 durable boundary'
require_current_boundary_matches_predecessor_rotation ||
  die 'the current durable boundary differs from the first helper-rotation link'
capture_rotation_durable_boundary ||
...
load_exact_predecessor_rotation_evidence ||
  die 'the first helper-rotation link changed under lock'
require_predecessor_rotation_global_residue_absent ||
  die 'first-rotation residue appeared while the grant was disabled'
require_preserved_rotation_durable_boundary ||
  die 'the stopped durable boundary changed under lock'
require_current_boundary_matches_predecessor_rotation ||
  die 'the frozen durable boundary diverged from the first helper-rotation link'
...
load_exact_predecessor_rotation_evidence ||
  die 'the first helper-rotation link changed after helper verification'
require_predecessor_rotation_global_residue_absent ||
  die 'first-rotation residue appeared during successor verification'
...
require_current_boundary_matches_predecessor_rotation ||
  die 'the durable boundary no longer matches the first helper-rotation link'
...
trap - EXIT
printf '%s\n' \
  'FetanAgent v3 second successor helper/release rotation completed. Transfer and the live executor remained disabled; no money moved.'
