#!/usr/bin/env bash
# One-use, root-console-only third rotation of an installed KemerBet v3 successor helper.
# The immutable v2-to-v3 evidence and first two helper-rotation links remain untouched;
# this operation appends one exact third link, keeps staging stopped, and never
# enables Transfer.
...
readonly PREDECESSOR_ROTATION_PARENT='/var/lib/fetanagent/kemerbet-readiness-v3-helper-rotation-v2'
readonly ROTATION_PARENT='/var/lib/fetanagent/kemerbet-readiness-v3-helper-rotation-v3'
...
readonly PREDECESSOR_SUDOERS_DISABLED='/etc/sudoers.d/.fetanagent-staging-deploy-helper.v3-rotation-v2-disabled'
readonly SUDOERS_DISABLED='/etc/sudoers.d/.fetanagent-staging-deploy-helper.v3-rotation-v3-disabled'
...
readonly PREDECESSOR_INSTALLING_HELPER='/usr/local/sbin/.fetanagent-staging-deploy-helper.v3-rotation-v2-installing'
...
readonly PREDECESSOR_ROLLBACK_HELPER='/usr/local/sbin/.fetanagent-staging-deploy-helper.v3-rotation-v2-rollback'
readonly PREDECESSOR_ROLLBACK_HELPER_PARTIAL="${PREDECESSOR_ROLLBACK_HELPER}.partial"
readonly INSTALLING_HELPER='/usr/local/sbin/.fetanagent-staging-deploy-helper.v3-rotation-v3-installing'
readonly INSTALLING_HELPER_PARTIAL="${INSTALLING_HELPER}.partial"
readonly ROLLBACK_HELPER='/usr/local/sbin/.fetanagent-staging-deploy-helper.v3-rotation-v3-rollback'
...
readonly PREDECESSOR_RELEASE='4bb491943fb88c50b86166184b929bdbe2698dc4'
readonly PREDECESSOR_HELPER_SHA256='05b0f2c8eb68716d20ad4878f1fff96c2f6a22e532e0b9c52a664e153b49e6fe'
readonly REVIEWED_SUCCESSOR_HELPER_SHA256='020b2b2d7eca153dffd72d7811d58c1a93e41edc24d1217cb459f5828e549b7b'
readonly CONFIRMATION='I-UNDERSTAND-THIS-APPENDS-THIRD-V3-HELPER-ROTATION-WITH-TRANSFER-DISABLED'
...
die() {
  printf 'FetanAgent v3 third helper rotation failed closed: %s\n' "$1" >&2
  exit 1
}
...
readonly STAGING_ROOT="/root/fetanagent-v3-helper-rotation-v3-$SUCCESSOR_RELEASE"
...
load_exact_predecessor_rotation_evidence() {
  ...
  inspection="$(env -i PATH="$SAFE_PATH" python3 -I - \
    "$FIRST_ROTATION_PARENT" "$PREDECESSOR_ROTATION_PARENT" \
    "$BASE_SUCCESSOR_RELEASE" "$FIRST_ROTATION_RELEASE" "$PREDECESSOR_RELEASE" \
    "$BASE_SUCCESSOR_HELPER_SHA256" "$FIRST_ROTATION_HELPER_SHA256" \
    "$PREDECESSOR_HELPER_SHA256" "$BASE_SUCCESSOR_INTENT_SHA256" \
...
if (
    any(release.fullmatch(value) is None for value in
        (base_release, first_release, predecessor_release))
    or len({base_release, first_release, predecessor_release}) != 3
    or any(sha.fullmatch(value) is None for value in
           (base_helper, first_helper, predecessor_helper, base_intent_sha,
            base_completion_sha, base_binding_v2_sha, base_old_helper_sha,
            base_binding_v3_sha))
    or len({base_helper, first_helper, predecessor_helper}) != 3
):
    reject()
...
archived_helper = exact_file(
    f'{predecessor_root}/predecessor-helper',
    0o400,
    2 * 1024 * 1024,
)
intent = intent_data.decode('ascii').splitlines()
...
if (
    ...
    or intent[0] != 'contract=fetanagent-kemerbet-readiness-v3-helper-rotation-v2'
    or intent[1] != 'state=authorized'
    or intent[2] != f'predecessor_release={first_release}'
    or intent[3] != f'successor_release={predecessor_release}'
    or intent[4] != f'predecessor_helper_sha256={first_helper}'
    ...
    or intent[14] != first_intent[11]
    or intent[15] != first_intent[12]
    or intent[16] != first_intent[13]
    or intent[17] != first_intent[14]
    or completion[:1] != intent[:1]
    or completion[1] != 'state=successor-installed'
    ...
    or intent_data != ('\n'.join(intent) + '\n').encode('ascii')
    or completion_data != ('\n'.join(completion) + '\n').encode('ascii')
    or hashlib.sha256(archived_helper).hexdigest() != first_helper
):
    reject()
...
cleanup() {
  ...
  if [[ "$status" -ne 0 && "$sudoers_disabled" == 'true' ]]; then
    ...
    printf '%s\n' \
      'FetanAgent v3 third helper rotation stopped with the deployment grant disabled. Whether the append-only record is pending or final, rerun this exact operation with the same successor release and helper digest from the root console; do not restore the grant manually.' >&2
  fi
  exit "$status"
}
...
require_predecessor_rotation_global_residue_absent ||
  die 'the second helper rotation still has global transaction residue'
...
load_exact_predecessor_rotation_evidence ||
  die 'the exact completed two-link helper-rotation chain is invalid'
require_predecessor_rotation_global_residue_absent ||
  die 'second-rotation residue appeared after lock acquisition'
require_stopped_no_transfer_boundary ||
  die 'rotation requires a stopped, disarmed, no-transient, exact Compose 5 durable boundary'
require_current_boundary_matches_predecessor_rotation ||
  die 'the current durable boundary differs from the second helper-rotation link'
capture_rotation_durable_boundary ||
...
load_exact_predecessor_rotation_evidence ||
  die 'the two-link helper-rotation chain changed under lock'
require_predecessor_rotation_global_residue_absent ||
  die 'second-rotation residue appeared while the grant was disabled'
require_preserved_rotation_durable_boundary ||
  die 'the stopped durable boundary changed under lock'
require_current_boundary_matches_predecessor_rotation ||
  die 'the frozen durable boundary diverged from the second helper-rotation link'
...
load_exact_predecessor_rotation_evidence ||
  die 'the two-link helper-rotation chain changed after helper verification'
require_predecessor_rotation_global_residue_absent ||
  die 'second-rotation residue appeared during successor verification'
...
require_current_boundary_matches_predecessor_rotation ||
  die 'the durable boundary no longer matches the second helper-rotation link'
...
trap - EXIT
printf '%s\n' \
  'FetanAgent v3 third successor helper/release rotation completed. Transfer and the live executor remained disabled; no money moved.'
