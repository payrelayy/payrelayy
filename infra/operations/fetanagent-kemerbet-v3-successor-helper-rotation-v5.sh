#!/usr/bin/env bash
# One-use, root-console-only fifth rotation of an installed KemerBet v3 successor helper.
# The immutable v2-to-v3 evidence and first four helper-rotation links remain untouched;
# this operation appends one exact fifth link, keeps staging stopped, and never
# enables Transfer.
...
readonly PREDECESSOR_ROTATION_PARENT='/var/lib/fetanagent/kemerbet-readiness-v3-helper-rotation-v4'
readonly ROTATION_PARENT='/var/lib/fetanagent/kemerbet-readiness-v3-helper-rotation-v5'
...
readonly PREDECESSOR_SUDOERS_DISABLED='/etc/sudoers.d/.fetanagent-staging-deploy-helper.v3-rotation-v4-disabled'
readonly SUDOERS_DISABLED='/etc/sudoers.d/.fetanagent-staging-deploy-helper.v3-rotation-v5-disabled'
...
readonly PREDECESSOR_INSTALLING_HELPER='/usr/local/sbin/.fetanagent-staging-deploy-helper.v3-rotation-v4-installing'
...
readonly PREDECESSOR_ROLLBACK_HELPER='/usr/local/sbin/.fetanagent-staging-deploy-helper.v3-rotation-v4-rollback'
readonly PREDECESSOR_ROLLBACK_HELPER_PARTIAL="${PREDECESSOR_ROLLBACK_HELPER}.partial"
readonly INSTALLING_HELPER='/usr/local/sbin/.fetanagent-staging-deploy-helper.v3-rotation-v5-installing'
readonly INSTALLING_HELPER_PARTIAL="${INSTALLING_HELPER}.partial"
readonly ROLLBACK_HELPER='/usr/local/sbin/.fetanagent-staging-deploy-helper.v3-rotation-v5-rollback'
...
readonly PREDECESSOR_RELEASE='874b8380a7e9f90806ebb1ad5c2958c1b245977f'
readonly PREDECESSOR_HELPER_SHA256='8ae567bb50581288600ef8058553fb411d6b04d1177a275b4e59bed936bb1db6'
readonly REVIEWED_SUCCESSOR_HELPER_SHA256='2fe840e137c435becc6179ba85cf00e86c1100b906da880b8f8e191a26cacd20'
readonly CONFIRMATION='I-UNDERSTAND-THIS-APPENDS-FIFTH-V3-HELPER-ROTATION-WITH-TRANSFER-DISABLED'
...
die() {
  printf 'FetanAgent v3 fifth helper rotation failed closed: %s\n' "$1" >&2
  exit 1
}
...
readonly STAGING_ROOT="/root/fetanagent-v3-helper-rotation-v5-$SUCCESSOR_RELEASE"
...
load_exact_predecessor_rotation_evidence() {
  ...
  inspection="$(env -i PATH="$SAFE_PATH" python3 -I - \
    ...
    "$SECOND_ROTATION_RELEASE" "$THIRD_ROTATION_RELEASE" "$PREDECESSOR_RELEASE" \
...
if (
    any(release.fullmatch(value) is None for value in
        (base_release, first_release, second_release, third_release, predecessor_release))
    or len({base_release, first_release, second_release, third_release, predecessor_release}) != 5
    or any(sha.fullmatch(value) is None for value in
           (base_helper, first_helper, second_helper, third_helper, predecessor_helper,
            base_intent_sha, base_completion_sha, base_binding_v2_sha,
            base_old_helper_sha, base_binding_v3_sha))
    or len({base_helper, first_helper, second_helper, third_helper, predecessor_helper}) != 5
):
    reject()
...
if (
    ...
    or intent[0] != 'contract=fetanagent-kemerbet-readiness-v3-helper-rotation-v4'
    or intent[1] != 'state=authorized'
    or intent[2] != f'predecessor_release={third_release}'
    or intent[3] != f'successor_release={predecessor_release}'
    or intent[4] != f'predecessor_helper_sha256={third_helper}'
    ...
    or intent[14] != third_intent[14]
    or intent[15] != third_intent[15]
    or intent[16] != third_intent[16]
    or intent[17] != third_intent[17]
    or completion[:1] != intent[:1]
    or completion[1] != 'state=successor-installed'
    ...
    or intent_data != ('\n'.join(intent) + '\n').encode('ascii')
    or completion_data != ('\n'.join(completion) + '\n').encode('ascii')
    or hashlib.sha256(archived_helper).hexdigest() != third_helper
):
    reject()
...
cleanup() {
  ...
  if [[ "$status" -ne 0 && "$sudoers_disabled" == 'true' ]]; then
    ...
    printf '%s\n' \
      'FetanAgent v3 fifth helper rotation stopped with the deployment grant disabled. Whether the append-only record is pending or final, rerun this exact operation with the same successor release and helper digest from the root console; do not restore the grant manually.' >&2
  fi
  exit "$status"
}
...
require_predecessor_rotation_global_residue_absent ||
  die 'one of the first four helper rotations still has global transaction residue'
...
load_exact_predecessor_rotation_evidence ||
  die 'the exact completed four-link helper-rotation chain is invalid'
require_predecessor_rotation_global_residue_absent ||
...
require_current_boundary_matches_predecessor_rotation ||
  die 'the current durable boundary differs from the fourth helper-rotation link'
capture_rotation_durable_boundary ||
...
load_exact_predecessor_rotation_evidence ||
  die 'the four-link helper-rotation chain changed under lock'
require_predecessor_rotation_global_residue_absent ||
...
require_current_boundary_matches_predecessor_rotation ||
  die 'the frozen durable boundary diverged from the fourth helper-rotation link'
...
load_exact_predecessor_rotation_evidence ||
  die 'the four-link helper-rotation chain changed after helper verification'
require_predecessor_rotation_global_residue_absent ||
...
require_current_boundary_matches_predecessor_rotation ||
  die 'the durable boundary no longer matches the fourth helper-rotation link'
...
trap - EXIT
printf '%s\n' \
  'FetanAgent v3 fifth successor helper/release rotation completed. Transfer and the live executor remained disabled; no money moved.'
