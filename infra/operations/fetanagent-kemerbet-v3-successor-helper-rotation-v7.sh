#!/usr/bin/env bash
# One-use, root-console-only seventh rotation of an installed KemerBet v3 successor helper.
# The immutable v2-to-v3 evidence and first six helper-rotation links remain untouched;
# this operation appends one exact seventh link, keeps staging stopped, and never
# enables Transfer.
...
readonly PREDECESSOR_ROTATION_PARENT='/var/lib/fetanagent/kemerbet-readiness-v3-helper-rotation-v6'
readonly ROTATION_PARENT='/var/lib/fetanagent/kemerbet-readiness-v3-helper-rotation-v7'
...
readonly PREDECESSOR_SUDOERS_DISABLED='/etc/sudoers.d/.fetanagent-staging-deploy-helper.v3-rotation-v6-disabled'
readonly SUDOERS_DISABLED='/etc/sudoers.d/.fetanagent-staging-deploy-helper.v3-rotation-v7-disabled'
...
readonly PREDECESSOR_INSTALLING_HELPER='/usr/local/sbin/.fetanagent-staging-deploy-helper.v3-rotation-v6-installing'
...
readonly PREDECESSOR_ROLLBACK_HELPER='/usr/local/sbin/.fetanagent-staging-deploy-helper.v3-rotation-v6-rollback'
readonly PREDECESSOR_ROLLBACK_HELPER_PARTIAL="${PREDECESSOR_ROLLBACK_HELPER}.partial"
readonly INSTALLING_HELPER='/usr/local/sbin/.fetanagent-staging-deploy-helper.v3-rotation-v7-installing'
readonly INSTALLING_HELPER_PARTIAL="${INSTALLING_HELPER}.partial"
readonly ROLLBACK_HELPER='/usr/local/sbin/.fetanagent-staging-deploy-helper.v3-rotation-v7-rollback'
...
readonly PREDECESSOR_RELEASE='1f7239c4116be509e9ca38c7343969d1773af692'
readonly PREDECESSOR_HELPER_SHA256='b3af405d303a123d4219ea8e6981ce447f9651abf83318879ebc3e579e318df1'
readonly REVIEWED_SUCCESSOR_HELPER_SHA256='6be7cbcb5e3715c9d45ecc86005bdcb9b41f6d4cb86fbdffaf47525f416defc8'
readonly CONFIRMATION='I-UNDERSTAND-THIS-APPENDS-SEVENTH-V3-HELPER-ROTATION-WITH-TRANSFER-DISABLED'
...
die() {
  printf 'FetanAgent v3 seventh helper rotation failed closed: %s\n' "$1" >&2
  exit 1
}
...
readonly STAGING_ROOT="/root/fetanagent-v3-helper-rotation-v7-$SUCCESSOR_RELEASE"
...
load_exact_predecessor_rotation_evidence() {
  ...
  inspection="$(env -i PATH="$SAFE_PATH" python3 -I - \
    ...
    "$FOURTH_ROTATION_PARENT" "$FIFTH_ROTATION_PARENT" "$PREDECESSOR_ROTATION_PARENT" \
    "$BASE_SUCCESSOR_RELEASE" "$FIRST_ROTATION_RELEASE" \
    "$SECOND_ROTATION_RELEASE" "$THIRD_ROTATION_RELEASE" \
    "$FOURTH_ROTATION_RELEASE" "$FIFTH_ROTATION_RELEASE" "$PREDECESSOR_RELEASE" \
...
if (
    ...
    or len({base_helper, first_helper, second_helper, third_helper, fourth_helper,
            fifth_helper, predecessor_helper}) != 7
):
    reject()
...
if (
    ...
    or intent[0] != 'contract=fetanagent-kemerbet-readiness-v3-helper-rotation-v6'
    or intent[1] != 'state=authorized'
    or intent[2] != f'predecessor_release={fifth_release}'
    or intent[3] != f'successor_release={predecessor_release}'
    or intent[4] != f'predecessor_helper_sha256={fifth_helper}'
    ...
    or intent[14] != fifth_intent[14]
    or intent[15] != fifth_intent[15]
    or intent[16] != fifth_intent[16]
    or intent[17] != fifth_intent[17]
    or completion[:1] != intent[:1]
    or completion[1] != 'state=successor-installed'
    ...
    or intent_data != ('\n'.join(intent) + '\n').encode('ascii')
    or completion_data != ('\n'.join(completion) + '\n').encode('ascii')
    or hashlib.sha256(archived_helper).hexdigest() != fifth_helper
):
    reject()
...
cleanup() {
  ...
  if [[ "$status" -ne 0 && "$sudoers_disabled" == 'true' ]]; then
    ...
    printf '%s\n' \
      'FetanAgent v3 seventh helper rotation stopped with the deployment grant disabled. Whether the append-only record is pending or final, rerun this exact operation with the same successor release and helper digest from the root console; do not restore the grant manually.' >&2
  fi
  exit "$status"
}
...
require_predecessor_rotation_global_residue_absent ||
  die 'one of the first six helper rotations still has global transaction residue'
...
load_exact_predecessor_rotation_evidence ||
  die 'the exact completed six-link helper-rotation chain is invalid'
require_predecessor_rotation_global_residue_absent ||
...
require_current_boundary_matches_predecessor_rotation ||
  die 'the current durable boundary differs from the sixth helper-rotation link'
capture_rotation_durable_boundary ||
...
load_exact_predecessor_rotation_evidence ||
  die 'the six-link helper-rotation chain changed under lock'
require_predecessor_rotation_global_residue_absent ||
...
require_current_boundary_matches_predecessor_rotation ||
  die 'the frozen durable boundary diverged from the sixth helper-rotation link'
...
load_exact_predecessor_rotation_evidence ||
  die 'the six-link helper-rotation chain changed after helper verification'
require_predecessor_rotation_global_residue_absent ||
...
require_current_boundary_matches_predecessor_rotation ||
  die 'the durable boundary no longer matches the sixth helper-rotation link'
...
trap - EXIT
printf '%s\n' \
  'FetanAgent v3 seventh successor helper/release rotation completed. Transfer and the live executor remained disabled; no money moved.'
