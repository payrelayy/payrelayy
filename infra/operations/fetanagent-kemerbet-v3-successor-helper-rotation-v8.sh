#!/usr/bin/env bash
# One-use, root-console-only eighth rotation of an installed KemerBet v3 successor helper.
# The immutable v2-to-v3 evidence and first seven helper-rotation links remain untouched;
# this operation appends one exact eighth link, keeps staging stopped, and never
# enables Transfer.
...
readonly PREDECESSOR_ROTATION_PARENT='/var/lib/fetanagent/kemerbet-readiness-v3-helper-rotation-v7'
readonly ROTATION_PARENT='/var/lib/fetanagent/kemerbet-readiness-v3-helper-rotation-v8'
...
readonly PREDECESSOR_SUDOERS_DISABLED='/etc/sudoers.d/.fetanagent-staging-deploy-helper.v3-rotation-v7-disabled'
readonly SUDOERS_DISABLED='/etc/sudoers.d/.fetanagent-staging-deploy-helper.v3-rotation-v8-disabled'
...
readonly PREDECESSOR_INSTALLING_HELPER='/usr/local/sbin/.fetanagent-staging-deploy-helper.v3-rotation-v7-installing'
...
readonly PREDECESSOR_ROLLBACK_HELPER='/usr/local/sbin/.fetanagent-staging-deploy-helper.v3-rotation-v7-rollback'
readonly PREDECESSOR_ROLLBACK_HELPER_PARTIAL="${PREDECESSOR_ROLLBACK_HELPER}.partial"
readonly INSTALLING_HELPER='/usr/local/sbin/.fetanagent-staging-deploy-helper.v3-rotation-v8-installing'
readonly INSTALLING_HELPER_PARTIAL="${INSTALLING_HELPER}.partial"
readonly ROLLBACK_HELPER='/usr/local/sbin/.fetanagent-staging-deploy-helper.v3-rotation-v8-rollback'
...
readonly PREDECESSOR_RELEASE='c73ba69d326be9c0b797b8fd9bbd77aa82cb89c9'
readonly PREDECESSOR_HELPER_SHA256='6be7cbcb5e3715c9d45ecc86005bdcb9b41f6d4cb86fbdffaf47525f416defc8'
readonly REVIEWED_SUCCESSOR_HELPER_SHA256='918fb4a5713a5d1fa5a3b214175e3a91dc6f5d505bd21d8fec85862416ac66cf'
readonly CONFIRMATION='I-UNDERSTAND-THIS-APPENDS-EIGHTH-V3-HELPER-ROTATION-WITH-TRANSFER-DISABLED'
...
die() {
  printf 'FetanAgent v3 eighth helper rotation failed closed: %s\n' "$1" >&2
  exit 1
}
...
readonly STAGING_ROOT="/root/fetanagent-v3-helper-rotation-v8-$SUCCESSOR_RELEASE"
...
load_exact_predecessor_rotation_evidence() {
  ...
  inspection="$(env -i PATH="$SAFE_PATH" python3 -I - \
    ...
    "$SIXTH_ROTATION_HELPER_SHA256" "$PREDECESSOR_HELPER_SHA256" \
    "$BASE_SUCCESSOR_INTENT_SHA256" \
...
if (
    ...
    or len({base_helper, first_helper, second_helper, third_helper, fourth_helper,
            fifth_helper, sixth_helper, predecessor_helper}) != 8
):
    reject()
...
if (
    ...
    or intent[0] != 'contract=fetanagent-kemerbet-readiness-v3-helper-rotation-v7'
    or intent[1] != 'state=authorized'
    or intent[2] != f'predecessor_release={sixth_release}'
    or intent[3] != f'successor_release={predecessor_release}'
    or intent[4] != f'predecessor_helper_sha256={sixth_helper}'
    ...
    or intent[14] != sixth_intent[14]
    or intent[15] != sixth_intent[15]
    or intent[16] != sixth_intent[16]
    or intent[17] != sixth_intent[17]
    or completion[:1] != intent[:1]
    or completion[1] != 'state=successor-installed'
    ...
    or intent_data != ('\n'.join(intent) + '\n').encode('ascii')
    or completion_data != ('\n'.join(completion) + '\n').encode('ascii')
    or hashlib.sha256(archived_helper).hexdigest() != sixth_helper
):
    reject()
...
cleanup() {
  ...
  if [[ "$status" -ne 0 && "$sudoers_disabled" == 'true' ]]; then
    ...
    printf '%s\n' \
      'FetanAgent v3 eighth helper rotation stopped with the deployment grant disabled. Whether the append-only record is pending or final, rerun this exact operation with the same successor release and helper digest from the root console; do not restore the grant manually.' >&2
  fi
  exit "$status"
}
...
require_predecessor_rotation_global_residue_absent ||
  die 'one of the first seven helper rotations still has global transaction residue'
...
load_exact_predecessor_rotation_evidence ||
  die 'the exact completed seven-link helper-rotation chain is invalid'
require_predecessor_rotation_global_residue_absent ||
...
require_current_boundary_matches_predecessor_rotation ||
  die 'the current durable boundary differs from the seventh helper-rotation link'
capture_rotation_durable_boundary ||
...
load_exact_predecessor_rotation_evidence ||
  die 'the seven-link helper-rotation chain changed under lock'
require_predecessor_rotation_global_residue_absent ||
...
require_current_boundary_matches_predecessor_rotation ||
  die 'the frozen durable boundary diverged from the seventh helper-rotation link'
...
load_exact_predecessor_rotation_evidence ||
  die 'the seven-link helper-rotation chain changed after helper verification'
require_predecessor_rotation_global_residue_absent ||
...
require_current_boundary_matches_predecessor_rotation ||
  die 'the durable boundary no longer matches the seventh helper-rotation link'
...
trap - EXIT
printf '%s\n' \
  'FetanAgent v3 eighth successor helper/release rotation completed. Transfer and the live executor remained disabled; no money moved.'
