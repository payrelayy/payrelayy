#!/usr/bin/env bash
# One-use, root-console-only ninth rotation of an installed KemerBet v3 successor helper.
# The immutable v2-to-v3 evidence and first eight helper-rotation links remain untouched;
# this operation appends one exact ninth link, keeps staging stopped, and never
# enables Transfer.
...
readonly PREDECESSOR_ROTATION_PARENT='/var/lib/fetanagent/kemerbet-readiness-v3-helper-rotation-v8'
readonly ROTATION_PARENT='/var/lib/fetanagent/kemerbet-readiness-v3-helper-rotation-v9'
...
readonly PREDECESSOR_SUDOERS_DISABLED='/etc/sudoers.d/.fetanagent-staging-deploy-helper.v3-rotation-v8-disabled'
readonly SUDOERS_DISABLED='/etc/sudoers.d/.fetanagent-staging-deploy-helper.v3-rotation-v9-disabled'
...
readonly PREDECESSOR_INSTALLING_HELPER='/usr/local/sbin/.fetanagent-staging-deploy-helper.v3-rotation-v8-installing'
...
readonly PREDECESSOR_ROLLBACK_HELPER='/usr/local/sbin/.fetanagent-staging-deploy-helper.v3-rotation-v8-rollback'
readonly PREDECESSOR_ROLLBACK_HELPER_PARTIAL="${PREDECESSOR_ROLLBACK_HELPER}.partial"
readonly INSTALLING_HELPER='/usr/local/sbin/.fetanagent-staging-deploy-helper.v3-rotation-v9-installing'
readonly INSTALLING_HELPER_PARTIAL="${INSTALLING_HELPER}.partial"
readonly ROLLBACK_HELPER='/usr/local/sbin/.fetanagent-staging-deploy-helper.v3-rotation-v9-rollback'
...
readonly PREDECESSOR_RELEASE='ede557b85ce5878b57f2a6abd0775c530276f46d'
readonly PREDECESSOR_HELPER_SHA256='918fb4a5713a5d1fa5a3b214175e3a91dc6f5d505bd21d8fec85862416ac66cf'
readonly REVIEWED_SUCCESSOR_HELPER_SHA256='d3284d1c268fdba227ff5628f2ac28f9e30375a8a85517e06258a97dfab5e4e1'
readonly CONFIRMATION='I-UNDERSTAND-THIS-APPENDS-NINTH-V3-HELPER-ROTATION-WITH-TRANSFER-DISABLED'
...
die() {
  printf 'FetanAgent v3 ninth helper rotation failed closed: %s\n' "$1" >&2
  exit 1
}
...
readonly STAGING_ROOT="/root/fetanagent-v3-helper-rotation-v9-$SUCCESSOR_RELEASE"
...
load_exact_predecessor_rotation_evidence() {
  ...
  inspection="$(env -i PATH="$SAFE_PATH" python3 -I - \
    ...
    "$SEVENTH_ROTATION_PARENT" "$PREDECESSOR_ROTATION_PARENT" \
    ...
    "$SEVENTH_ROTATION_RELEASE" "$PREDECESSOR_RELEASE" \
...
if (
    ...
    or len({base_helper, first_helper, second_helper, third_helper, fourth_helper,
            fifth_helper, sixth_helper, seventh_helper, predecessor_helper}) != 9
):
    reject()
...
if (
    ...
    or intent[0] != 'contract=fetanagent-kemerbet-readiness-v3-helper-rotation-v8'
    or intent[1] != 'state=authorized'
    or intent[2] != f'predecessor_release={seventh_release}'
    or intent[3] != f'successor_release={predecessor_release}'
    or intent[4] != f'predecessor_helper_sha256={seventh_helper}'
    ...
    or intent[14] != seventh_intent[14]
    or intent[15] != seventh_intent[15]
    or intent[16] != seventh_intent[16]
    or intent[17] != seventh_intent[17]
    or completion[:1] != intent[:1]
    or completion[1] != 'state=successor-installed'
    ...
    or intent_data != ('\n'.join(intent) + '\n').encode('ascii')
    or completion_data != ('\n'.join(completion) + '\n').encode('ascii')
    or hashlib.sha256(archived_helper).hexdigest() != seventh_helper
):
    reject()
...
cleanup() {
  ...
  if [[ "$status" -ne 0 && "$sudoers_disabled" == 'true' ]]; then
    ...
    printf '%s\n' \
      'FetanAgent v3 ninth helper rotation stopped with the deployment grant disabled. Whether the append-only record is pending or final, rerun this exact operation with the same successor release and helper digest from the root console; do not restore the grant manually.' >&2
  fi
  exit "$status"
}
...
require_predecessor_rotation_global_residue_absent ||
  die 'one of the first eight helper rotations still has global transaction residue'
...
load_exact_predecessor_rotation_evidence ||
  die 'the exact completed eight-link helper-rotation chain is invalid'
require_predecessor_rotation_global_residue_absent ||
...
require_current_boundary_matches_predecessor_rotation ||
  die 'the current durable boundary differs from the eighth helper-rotation link'
capture_rotation_durable_boundary ||
...
load_exact_predecessor_rotation_evidence ||
  die 'the eight-link helper-rotation chain changed under lock'
require_predecessor_rotation_global_residue_absent ||
...
require_current_boundary_matches_predecessor_rotation ||
  die 'the frozen durable boundary diverged from the eighth helper-rotation link'
...
load_exact_predecessor_rotation_evidence ||
  die 'the eight-link helper-rotation chain changed after helper verification'
require_predecessor_rotation_global_residue_absent ||
...
require_current_boundary_matches_predecessor_rotation ||
  die 'the durable boundary no longer matches the eighth helper-rotation link'
...
trap - EXIT
printf '%s\n' \
  'FetanAgent v3 ninth successor helper/release rotation completed. Transfer and the live executor remained disabled; no money moved.'
