#!/usr/bin/env bash
# One-use, root-console-only rotation of an installed KemerBet v3 successor helper.
# The immutable v2-to-v3 evidence remains untouched; this operation adds one exact
# successor link, keeps the staging runtime stopped, and never enables Transfer.
...
readonly ROTATION_PARENT='/var/lib/fetanagent/kemerbet-readiness-v3-helper-rotation'
...
readonly SUDOERS_DISABLED='/etc/sudoers.d/.fetanagent-staging-deploy-helper.v3-rotation-disabled'
readonly INSTALLING_HELPER='/usr/local/sbin/.fetanagent-staging-deploy-helper.v3-rotation-installing'
readonly INSTALLING_HELPER_PARTIAL="${INSTALLING_HELPER}.partial"
readonly ROLLBACK_HELPER='/usr/local/sbin/.fetanagent-staging-deploy-helper.v3-rotation-rollback'
...
readonly PREDECESSOR_RELEASE='de14588d4e5b8ee9e80a1a667f2e4d59ef6a62e3'
readonly PREDECESSOR_HELPER_SHA256='e94dfdcfe90ff6021446fc66e2850ae13198b03d9e2210f454181ab00177f97d'
readonly REVIEWED_SUCCESSOR_HELPER_SHA256='f98047953fb9249d7dbcd13be6cf1a145b53a4952a760b36d5ba8bfab2f36f82'
readonly CONFIRMATION='I-UNDERSTAND-THIS-APPENDS-ONE-V3-HELPER-ROTATION-WITH-TRANSFER-DISABLED'
...
die() {
  printf 'FetanAgent v3 helper rotation failed closed: %s\n' "$1" >&2
  exit 1
}
...
readonly STAGING_ROOT="/root/fetanagent-v3-helper-rotation-$SUCCESSOR_RELEASE"
...
[[ "$SUCCESSOR_RELEASE" =~ ^[0-9a-f]{40}$ && "$SUCCESSOR_RELEASE" != "$PREDECESSOR_RELEASE" ]] ||
  die 'the successor must be a distinct full lowercase Git commit SHA'
...
load_exact_base_successor_evidence() {
  ...
  inspection="$(env -i PATH="$SAFE_PATH" python3 -I - \
    "$BASE_PARENT" "$SOURCE" "$PREDECESSOR_RELEASE" "$PREDECESSOR_HELPER_SHA256" \
...
cleanup() {
  ...
  if [[ "$status" -ne 0 && "$sudoers_disabled" == 'true' ]]; then
    ...
    printf '%s\n' \
      'FetanAgent v3 helper rotation stopped with the deployment grant disabled. Whether the append-only record is pending or final, rerun this exact operation with the same successor release and helper digest from the root console; do not restore the grant manually.' >&2
  fi
  exit "$status"
}
...
trap - EXIT
printf '%s\n' \
  'FetanAgent v3 successor helper/release rotation completed. Transfer and the live executor remained disabled; no money moved.'
