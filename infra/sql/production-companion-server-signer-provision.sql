...
select :'confirmed_project_ref' = 'xzztugbgtulptnbpoelr' as production_target_confirmed
\gset
\if :production_target_confirmed
\else
  \warn 'The exact production project must be confirmed by the deployment workflow.'
...
select pg_catalog.pg_advisory_xact_lock(
  pg_catalog.hashtextextended('fetanagent:production:companion-server-signer:v1', 0)
);
...
select :'companion_signer_id'
    ...
    and :'companion_signer_key_id' = 'companion-server-production-v1'
