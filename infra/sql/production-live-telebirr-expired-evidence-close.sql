...
\else
  \warn 'The expired-evidence close contract is invalid.'
...
select exists (
  select 1
   ...
   where authority.request_key = :'historical_completion_request_key'::uuid
) as authority_exists
\gset recovery_

\if :recovery_authority_exists
  select closed.*
  ...
  \gset close_
\else
  select disabled.verifier_login,
         disabled.terminated_session_count
...
select pg_catalog.jsonb_build_object(
  'schemaVersion', 1,
  'operation', 'live_telebirr_expired_evidence_close',
  'deploymentTarget', 'production',
  'authorityRecorded', :'recovery_authority_exists'::boolean,
  'closeReasonCode', :'close_reason_code',
  'verifierLoginDisabled', runtime.verifier_login_or_password_roles = 0,
