...
create temp table reviewed_parser_refinement_retry_result
on commit drop
as
select *
  from app.retry_reviewed_private_telebirr_receipt_parser_refinement(
    :'reviewed_main_commit_sha',
    'reviewed_receipt_parser_refinement_retry_no_credit'
  );

do $validate_redacted_parser_refinement_retry_result$
begin
  if (select pg_catalog.count(*) from reviewed_parser_refinement_retry_result) <> 1
    or not exists (
      select 1
        from reviewed_parser_refinement_retry_result result
       where result.parser_retry_count = 1
         ...
         and result.source_parser_refinement_review_count =
    ...
    raise exception 'The redacted parser-refinement retry result is invalid.';
  end if;
end;
$validate_redacted_parser_refinement_retry_result$;
...
with exact_retry as materialized (
  select retry.*
   ...
   where retry.reason_code = 'reviewed_receipt_parser_refinement_retry_no_credit'
  ...
  select
    ...
    (select pg_catalog.count(*)
      ...
      )) as source_parser_refinement_reviews,
    (select pg_catalog.count(*)
...
select pg_catalog.jsonb_build_object(
  'schemaVersion', 1,
  'operation', 'reviewed_receipt_parser_refinement_retry',
  'deploymentTarget', 'production',
  'created', result.created,
  'parserRetryCount', state.retries,
  ...
  'sourceParserRefinementReviewCount', state.source_parser_refinement_reviews,
