...
create temp table reviewed_provider_origin_retry_result
on commit drop
as
select *
  from app.retry_reviewed_private_telebirr_receipt_provider_origin(
    :'reviewed_main_commit_sha',
    'reviewed_receipt_provider_origin_retry_no_credit'
  );

do $validate_redacted_provider_origin_retry_result$
begin
  if (select pg_catalog.count(*) from reviewed_provider_origin_retry_result) <> 1
    or not exists (
      select 1
        from reviewed_provider_origin_retry_result result
       where result.origin_retry_count = 1
         ...
         and result.source_provider_identity_review_count =
    ...
    raise exception 'The redacted provider-origin retry result is invalid.';
  end if;
end;
$validate_redacted_provider_origin_retry_result$;
...
with exact_retry as materialized (
  select retry.*
   ...
   where retry.reason_code = 'reviewed_receipt_provider_origin_retry_no_credit'
  ...
  select
    ...
    (select pg_catalog.count(*)
      ...
      where staged.signed_observation -> 'body' -> 'facts' ->> 'reviewReason' =
            'unknown_layout_provider_identity') as source_provider_identity_reviews,
    (select pg_catalog.count(*)
...
select pg_catalog.jsonb_build_object(
  'schemaVersion', 1,
  'operation', 'reviewed_receipt_provider_origin_retry',
  'deploymentTarget', 'production',
  'created', result.created,
  'originRetryCount', state.retries,
  ...
  'sourceProviderIdentityReviewCount', state.source_provider_identity_reviews,
