...
create temp table reviewed_receipt_cell_binding_retry_result
on commit drop
as
select *
  from app.retry_reviewed_private_telebirr_receipt_cell_binding(
    :'reviewed_main_commit_sha',
    'reviewed_receipt_cell_binding_retry_no_credit'
  );

do $validate_redacted_receipt_cell_binding_retry_result$
begin
  if (select pg_catalog.count(*) from reviewed_receipt_cell_binding_retry_result) <> 1
    or not exists (
      select 1
        from reviewed_receipt_cell_binding_retry_result result
       where result.receipt_cell_binding_retry_count = 1
         ...
         and result.source_receipt_cell_binding_review_count =
    ...
    raise exception 'The redacted receipt-cell-binding retry result is invalid.';
  end if;
end;
$validate_redacted_receipt_cell_binding_retry_result$;
...
with exact_retry as materialized (
  select retry.*
   ...
   where retry.reason_code = 'reviewed_receipt_cell_binding_retry_no_credit'
  ...
  select
    ...
    (select pg_catalog.count(*)
      ...
      where staged.signed_observation -> 'body' -> 'facts' ->> 'reviewReason' =
            'unknown_layout_invoice_number') as source_receipt_cell_binding_reviews,
    (select pg_catalog.count(*)
...
select pg_catalog.jsonb_build_object(
  'schemaVersion', 1,
  'operation', 'reviewed_receipt_cell_binding_retry',
  'deploymentTarget', 'production',
  'created', result.created,
  'cellBindingRetryCount', state.retries,
  ...
  'sourceReceiptCellBindingReviewCount', state.source_receipt_cell_binding_reviews,
