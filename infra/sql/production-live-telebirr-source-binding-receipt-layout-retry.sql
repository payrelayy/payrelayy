...
create temp table reviewed_source_binding_layout_retry_result
on commit drop
as
select *
  from app.retry_reviewed_private_telebirr_source_binding_receipt_layout(
    :'reviewed_main_commit_sha',
    'reviewed_source_binding_receipt_layout_retry_no_credit'
  );

do $validate_redacted_source_binding_layout_retry_result$
begin
  if (select pg_catalog.count(*) from reviewed_source_binding_layout_retry_result) <> 1
    or not exists (
      select 1
        from reviewed_source_binding_layout_retry_result result
       where result.layout_retry_count = 1
         ...
         and result.source_layout_evidence_count between 1
             and result.source_shadow_attempt_count
    ...
    raise exception 'The redacted receipt-layout retry result is invalid.';
  end if;
end;
$validate_redacted_source_binding_layout_retry_result$;

with exact_retry as materialized (
  select retry.*,
         source_window.source_recovery_request_key
    from app.private_telebirr_shadow_source_binding_layout_retries retry
    join app.private_telebirr_shadow_source_binding_window_retries source_window
      on source_window.retry_request_key = retry.source_window_retry_request_key
  ...
  select
    ...
    (select pg_catalog.count(*)
       ...
       join exact_retry retry
         on retry.source_shadow_proof_request_id = attempt.shadow_proof_request_id
      where staged.signed_observation -> 'body' -> 'facts' ->> 'reviewReason' in (
        'unknown_layout',
...
select pg_catalog.jsonb_build_object(
  'schemaVersion', 1,
  'operation', 'reviewed_source_binding_receipt_layout_retry',
  'deploymentTarget', 'production',
  'created', result.created,
  'layoutRetryCount', state.retries,
  'shadowRequestCount', state.shadow_requests,
  'sourceShadowAttemptCount', state.source_attempts,
  'sourceStagedEvidenceCount', state.source_staged_evidence,
  'sourceLayoutEvidenceCount', state.source_layout_evidence,
