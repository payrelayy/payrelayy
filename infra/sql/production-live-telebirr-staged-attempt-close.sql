...
\else
  \warn 'The staged-attempt close contract is invalid.'
...
select exists (
  select 1 from app.private_live_telebirr_historical_completion_authorities authority
   where authority.request_key = :'staged_completion_request_key'::uuid
     and authority.reason_code = 'expired_attempt_staged_evidence_completion'
) as authority_exists
\gset recovery_

\if :recovery_authority_exists
  select closed.* from app.close_private_live_telebirr_historical_completion(
    :'staged_completion_request_key'::uuid, :'close_reason_code'
  ) closed
  \gset close_
\else
...
select pg_catalog.jsonb_build_object(
  'schemaVersion', 1,
  'operation', 'live_telebirr_staged_attempt_close',
