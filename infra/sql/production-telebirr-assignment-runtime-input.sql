...
select :'pilot_revision_id' ~
   ...
   and :'activation_epoch' ~ '^[1-9][0-9]{0,17}$'
  as deployment_binding_canonical
\gset
\if :deployment_binding_canonical
\else
  \warn 'The production TeleBirr deployment binding is malformed.'
  select 1 / 0 as rejected;
\endif

select count(*) = 1 as exact_active_authority
...
where control.control_key = 'trusted_telebirr_financial_authority'
  ...
  and authority.active_from = pilot.active_from
  and authority.expires_at = pilot.expires_at
  ...
  and not exists (
    select 1
    from app.private_trusted_telebirr_emergency_disable_intents intent
    where intent.expected_epoch = authority.epoch
  )
\gset
\if :exact_active_authority
\else
  \warn 'Exactly one current production TeleBirr activation epoch is required.'
  select 1 / 0 as rejected;
\endif

select count(*) = 5 as exact_financial_switch_contract
...
where (
  ...
  or (
    ...
    and feature_switch.settings = pg_catalog.jsonb_build_object(
      'contract_version', 1,
      'pilot_revision_id', :'pilot_revision_id'::uuid,
...
\gset
\if :exact_financial_switch_contract
\else
  \warn 'The production TeleBirr activation switch contract is not exact.'
  select 1 / 0 as rejected;
\endif
...
\else
  \warn 'The active production pilot does not contain exactly five Players.'
  select 1 / 0 as rejected;
\endif
...
\else
  \warn 'The production pilot receiver and assignment signer binding is not exact.'
  select 1 / 0 as rejected;
\endif
...
\else
  \warn 'Exactly one current enrolled production TeleBirr verifier device is required.'
  select 1 / 0 as rejected;
\endif
