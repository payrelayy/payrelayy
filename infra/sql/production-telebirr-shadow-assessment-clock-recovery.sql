...
\else
  \warn 'The assessment-clock recovery identifiers are invalid.'
  select 1 / 0 as rejected;
\endif

select pg_catalog.count(*) = 1 as exact_source_ready
 ...
 where retry.retry_request_key = :'source_authority_retry_request_key'::uuid
   ...
   and proof.authority_deadline_retry_source_id = retry.source_shadow_proof_request_id
   and proof.assessment_clock_retry_source_id is null
\gset
\if :exact_source_ready
\else
  \warn 'The exact terminal authority-deadline source proof is unavailable.'
  select 1 / 0 as rejected;
\endif

select retry.replacement_shadow_proof_request_id::text as source_shadow_proof_request_id,
       retry.replacement_shadow_verification_job_id::text as source_shadow_verification_job_id
  from app.private_telebirr_shadow_authority_deadline_retries retry
 where retry.retry_request_key = :'source_authority_retry_request_key'::uuid
   and retry.pilot_revision_id = :'pilot_revision_id'::uuid
\gset
...
select *
  from app.retry_private_telebirr_shadow_after_assessment_clock_fix(
...
with exact_retry as materialized (
  select retry.*
   ...
   where retry.retry_request_key = :'retry_request_key'::uuid
     and retry.source_authority_retry_request_key =
         :'source_authority_retry_request_key'::uuid
     and retry.pilot_revision_id = :'pilot_revision_id'::uuid
  ...
  select proof.*
   ...
   for share of proof
), exact_original as materialized (
  select proof.*
    from app.private_telebirr_shadow_proof_requests proof
    join exact_retry retry on retry.original_shadow_proof_request_id = proof.id
   for share of proof
...
select (select count(*) from exact_retry) = 1
   ...
   and (select count(*) from exact_original) = 1
   ...
   and (select pg_catalog.bool_and(
          retry.retry_expires_at = retry.authorized_at + interval '12 hours'
          and retry.retry_expires_at = :'recovery_retry_expires_at'::timestamptz
          and retry.assessment_submitted_at = original.submitted_at
          and retry.assessment_submitted_at < retry.authorized_at
        ) from exact_retry retry cross join exact_original original)
   and (select pg_catalog.bool_and(
          ...
          and replacement.assessment_clock_retry_source_id = source.id
          ...
          cross join exact_source source
          cross join exact_replacement replacement)
   and (select pg_catalog.bool_and(
          app.private_telebirr_shadow_assessment_clock_retry_is_valid(
            replacement.id,
            retry.retry_request_key
          )
          and app.private_telebirr_shadow_assessment_submitted_at(replacement.id) =
...
\else
  \warn 'The production TeleBirr assessment-clock recovery postcondition failed.'
  select 1 / 0 as rejected;
\endif
...
select pg_catalog.jsonb_build_object(
  'schemaVersion', 1,
  'operation', 'telebirr_shadow_assessment_clock_recovery',
  'deploymentTarget', 'production',
  'retryState', 'armed',
  'reviewWindowHours', 12,
  'originalAssessmentClockPreserved', true,
  'priorEvidencePreserved', true,
  'priorOutcomePreserved', true,
