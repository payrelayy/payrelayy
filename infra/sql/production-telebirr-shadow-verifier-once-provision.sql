...
select :'source_live_verification_job_id' = 'not-applicable'
   ...
   and (
     ...
     or app.private_telebirr_shadow_parser_refinement_retry_is_valid(
       :'target_shadow_proof_request_id'::uuid,
       nullif(:'recovery_request_key', 'not-applicable')::uuid
     )
     or app.private_telebirr_shadow_receipt_alias_retry_is_valid(
       :'target_shadow_proof_request_id'::uuid,
       nullif(:'recovery_request_key', 'not-applicable')::uuid
     )
         ...
         where attempt.shadow_proof_request_id = shadow_proof.id
           ...
           and (
             ...
             or (
              ...
              or (
                app.private_telebirr_shadow_source_binding_window_retry_is_valid(
                  shadow_proof.id,
                  nullif(:'recovery_request_key', 'not-applicable')::uuid
                )
                and exists (
              ...
              or (
                app.private_telebirr_shadow_parser_refinement_retry_is_valid(
                  shadow_proof.id,
                  nullif(:'recovery_request_key', 'not-applicable')::uuid
                )
                and exists (
              ...
              or (
                app.private_telebirr_shadow_receipt_shape_diag_retry_is_valid(
                  shadow_proof.id,
                  nullif(:'recovery_request_key', 'not-applicable')::uuid
                )
                and exists (
...
select (select count(*) from locked_feature_switches) = 7
   ...
   and pg_catalog.to_regprocedure(
    ...
    and pg_catalog.to_regprocedure(
          'app.private_telebirr_shadow_receipt_shape_diag_retry_is_valid(uuid,uuid)'
        ) is not null
    and pg_catalog.to_regclass(
