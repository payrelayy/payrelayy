-- One final retry of the same retained no-money TeleBirr shadow request after an infrastructure-
-- only failure. The first retry already proved semantic equivalence before rebinding to the current
-- dry-run pilot. This transition cannot change that pilot, profile, protected reference, live or
-- shadow identity, create financial authority, or run after any live/shadow attempt or evidence.
...
alter table app.private_telebirr_shadow_proof_requests
  add column infrastructure_retry_request_key uuid,
  add column infrastructure_retry_request_digest text,
  add column infrastructure_retry_reason_code text,
  add column infrastructure_retry_original_expires_at timestamptz,
  add column infrastructure_retried_at timestamptz,
  add constraint private_tbirr_shadow_infra_retry_key_v4_check check (
    infrastructure_retry_request_key is null
    or infrastructure_retry_request_key::text
      ~ '^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
  ),
  add constraint private_tbirr_shadow_infra_retry_digest_check check (
    infrastructure_retry_request_digest is null
    or infrastructure_retry_request_digest ~ '^sha256:[0-9a-f]{64}$'
  ),
  add constraint private_tbirr_shadow_infra_retry_reason_check check (
    infrastructure_retry_reason_code is null
    or infrastructure_retry_reason_code = 'expired_shadow_infrastructure_retry_no_credit'
  ),
...
create unique index private_tbirr_shadow_infra_retry_key_idx
  on app.private_telebirr_shadow_proof_requests (infrastructure_retry_request_key)
  where infrastructure_retry_request_key is not null;
create unique index private_tbirr_shadow_infra_retry_digest_idx
  on app.private_telebirr_shadow_proof_requests (infrastructure_retry_request_digest)
  where infrastructure_retry_request_digest is not null;

create function app.private_telebirr_shadow_infrastructure_retry_digest(
  p_infrastructure_retry_request_key uuid,
  ...
  p_infrastructure_retried_at timestamptz,
  p_infrastructure_retry_expires_at timestamptz,
  p_reason_code text
)
returns text
language plpgsql
...
as $$
begin
  if p_infrastructure_retry_request_key is null
    ...
    or p_infrastructure_retried_at is null
    or p_infrastructure_retry_expires_at is null
    or p_reason_code is distinct from
       'expired_shadow_infrastructure_retry_no_credit' then
    raise exception 'The shadow infrastructure-retry digest input is invalid.';
  end if;

  return app.private_live_deposit_pilot_sha256(
    'fetanagent:telebirr:expired-shadow:infrastructure-retry:v1'
      || '|request_key=' || p_infrastructure_retry_request_key::text
      ...
      || '|infrastructure_retried_at_us=' || (
        extract(epoch from p_infrastructure_retried_at) * 1000000
      )::bigint::text
      || '|infrastructure_retry_expires_at_us=' || (
        extract(epoch from p_infrastructure_retry_expires_at) * 1000000
      )::bigint::text
...
create function app.enforce_private_telebirr_shadow_infrastructure_retry_only()
returns trigger
language plpgsql
security definer
set search_path = pg_catalog
...
begin
  if tg_op <> 'UPDATE'
    ...
    or old.retry_prior_pilot_revision_id is null
    or old.retry_prior_receiver_profile_id is null
    or old.retry_prior_configuration_digest is null
    or old.retry_original_expires_at is null
    or old.retried_at is null
    or old.infrastructure_retry_request_key is not null
    or old.infrastructure_retry_request_digest is not null
    or old.infrastructure_retry_reason_code is not null
    or old.infrastructure_retry_original_expires_at is not null
    or old.infrastructure_retried_at is not null
    or old.expires_at > pg_catalog.clock_timestamp()
    or new.infrastructure_retry_request_key is null
    or new.infrastructure_retry_request_digest is null
    or new.infrastructure_retry_reason_code is distinct from
       'expired_shadow_infrastructure_retry_no_credit'
    or new.infrastructure_retry_original_expires_at is distinct from old.expires_at
    or new.infrastructure_retried_at is null
    or new.infrastructure_retried_at < old.expires_at
    or new.expires_at <= new.infrastructure_retried_at + interval '60 seconds'
    or new.expires_at > new.infrastructure_retried_at + interval '5 minutes'
    or (
  ...
  expected_digest := app.private_telebirr_shadow_infrastructure_retry_digest(
    new.infrastructure_retry_request_key,
    new.id,
    ...
    new.infrastructure_retry_original_expires_at,
    new.infrastructure_retried_at,
    new.expires_at,
    new.infrastructure_retry_reason_code
  );

  if new.infrastructure_retry_request_digest is distinct from expected_digest then
    raise exception 'The shadow infrastructure-retry digest does not match.';
  end if;
...
create trigger private_telebirr_shadow_proofs_delete_immutable
before delete on app.private_telebirr_shadow_proof_requests
for each row execute function app.enforce_private_telebirr_shadow_proof_retry_only();
create trigger private_telebirr_shadow_proofs_first_retry_only
before update on app.private_telebirr_shadow_proof_requests
for each row
when (old.retry_request_key is null)
execute function app.enforce_private_telebirr_shadow_proof_retry_only();
create trigger private_telebirr_shadow_proofs_infrastructure_retry_only
before update on app.private_telebirr_shadow_proof_requests
for each row
when (old.retry_request_key is not null)
execute function app.enforce_private_telebirr_shadow_infrastructure_retry_only();

create function app.retry_expired_private_telebirr_shadow_after_infrastructure_failure(
  p_shadow_proof_request_id uuid,
  p_source_live_verification_job_id uuid,
  p_target_pilot_revision_id uuid,
  p_infrastructure_retry_request_key uuid,
...
begin
  if session_user <> 'postgres'
    ...
    or p_source_live_verification_job_id is null
    or p_target_pilot_revision_id is null
    or p_infrastructure_retry_request_key is null
    or p_infrastructure_retry_request_key::text
       !~ '^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
    or p_reason_code is distinct from
       'expired_shadow_infrastructure_retry_no_credit'
    or pg_catalog.current_setting('transaction_isolation') <> 'read committed' then
    raise exception 'The shadow infrastructure no-credit retry request is invalid.';
  end if;
  ...
  select candidate.*
   ...
   where candidate.id = p_shadow_proof_request_id
      or candidate.infrastructure_retry_request_key =
           p_infrastructure_retry_request_key
  ...
  if shadow_request.infrastructure_retry_request_key is not null then
    infrastructure_retry_digest :=
      app.private_telebirr_shadow_infrastructure_retry_digest(
    ...
    if shadow_request.id is distinct from p_shadow_proof_request_id
      ...
      or shadow_request.infrastructure_retry_request_key
           is distinct from p_infrastructure_retry_request_key
      or shadow_request.infrastructure_retry_reason_code is distinct from p_reason_code
      or shadow_request.infrastructure_retry_request_digest
           is distinct from infrastructure_retry_digest then
      raise exception 'The shadow infrastructure no-credit retry replay conflicts.';
    end if;
  ...
  if locked_switch_count <> 7
    ...
    raise exception 'The shadow request is not safely retryable after infrastructure failure.';
  end if;
  ...
  if retry_until <= authorized_at + interval '60 seconds' then
    raise exception 'The shadow infrastructure no-credit retry window is unavailable.';
  end if;
  ...
  perform pg_catalog.set_config(
    'app.private_telebirr_shadow_infrastructure_retry', 'on', true
  );
  update app.private_telebirr_shadow_proof_requests request
     set expires_at = retry_until,
         infrastructure_retry_request_key = p_infrastructure_retry_request_key,
         infrastructure_retry_request_digest = infrastructure_retry_digest,
         infrastructure_retry_reason_code = p_reason_code,
         infrastructure_retry_original_expires_at = shadow_request.expires_at,
         infrastructure_retried_at = authorized_at
   where request.id = shadow_request.id
     and request.infrastructure_retry_request_key is null
     and request.expires_at <= authorized_at
  returning * into updated_request;
  perform pg_catalog.set_config(
    'app.private_telebirr_shadow_infrastructure_retry', 'off', true
  );

  if updated_request.id is null then
    raise exception 'The shadow infrastructure no-credit retry changed before commit.';
  end if;
...
alter function app.private_telebirr_shadow_infrastructure_retry_digest(
  uuid, uuid, uuid, uuid, uuid, uuid, uuid, text, text,
  uuid, uuid, text, timestamptz, timestamptz, timestamptz, text
) owner to postgres;
alter function app.enforce_private_telebirr_shadow_infrastructure_retry_only()
  owner to postgres;
alter function app.retry_expired_private_telebirr_shadow_after_infrastructure_failure(
...
revoke all on function
  ...
  app.enforce_private_telebirr_shadow_infrastructure_retry_only(),
  app.retry_expired_private_telebirr_shadow_after_infrastructure_failure(
    uuid, uuid, uuid, uuid, text
  )
...
comment on function app.retry_expired_private_telebirr_shadow_after_infrastructure_failure(
  uuid, uuid, uuid, uuid, text
) is
  'Postgres-only, exactly-once reopening of the same expired and wholly untouched TeleBirr shadow request on the same armed dry-run pilot after an infrastructure-only failure; no credit or money authority is created.';
comment on function app.private_telebirr_shadow_infrastructure_retry_digest(
  uuid, uuid, uuid, uuid, uuid, uuid, uuid, text, text,
  uuid, uuid, text, timestamptz, timestamptz, timestamptz, text
) is
  'Private canonical digest binding the complete retained live/shadow lineage, both earlier transition digests, unchanged target pilot/profile, and final infrastructure-only no-credit retry window.';
comment on function app.enforce_private_telebirr_shadow_infrastructure_retry_only() is
  'Keeps a once-retried TeleBirr shadow request immutable except for one fully bound postgres-only infrastructure-recovery window on the unchanged dry-run pilot.';
comment on table app.private_telebirr_shadow_proof_requests is
  'Immutable no-money TeleBirr shadow requests. One wholly untouched request may be rebound once to a fresh identical pilot and reopened once more on that unchanged pilot only after an infrastructure-only failure.';
