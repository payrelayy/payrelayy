-- Prepare one append-only, no-money recovery from the exact terminal live TeleBirr
-- review_required/source_unavailable lineage into the existing dry-run shadow verifier.
--
-- The original live proof, its original job, the network-retry job, both terminal outcomes, and
-- every signed attempt/evidence row remain immutable.  The recovery creates only one short-lived
-- shadow request under a fresh, semantically identical dry-run pilot.  It cannot create a claim,
-- reservation, settlement receipt, execution job, KemerBet action, or financial authority.
...
create table app.private_live_telebirr_source_recoveries (
  ...
  target_receiver_profile_id uuid not null
    references app.private_live_telebirr_receiver_profiles (id) on delete restrict,
  source_attempt_count integer not null check (source_attempt_count = 4),
  source_assignment_transcript_count integer not null
    check (source_assignment_transcript_count = 2),
  source_assignment_delivery_count integer not null
    check (source_assignment_delivery_count = 2),
  source_device_evidence_count integer not null
    check (source_device_evidence_count = 2),
  ...
  reason_code text not null
    check (reason_code = 'terminal_source_unavailable_recovery_no_credit'),
  created_at timestamptz not null default pg_catalog.clock_timestamp(),
  constraint private_live_tbirr_source_recovery_request_v4_check check (
    recovery_request_key::text
      ~ '^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
  ),
  constraint private_live_tbirr_source_recovery_jobs_distinct_check check (
    root_live_verification_job_id <> terminal_live_verification_job_id
  ),
  constraint private_live_tbirr_source_recovery_outcomes_distinct_check check (
    root_live_outcome_id <> terminal_live_outcome_id
  ),
  constraint private_live_tbirr_source_recovery_pilots_distinct_check check (
    source_pilot_revision_id <> target_pilot_revision_id
  ),
  constraint private_live_tbirr_source_recovery_window_check check (
    recovery_expires_at > authorized_at + interval '60 seconds'
    and recovery_expires_at <= authorized_at + interval '5 minutes'
  ),
  constraint private_live_tbirr_source_recovery_shadow_fkey foreign key (
...
create index private_live_tbirr_source_recovery_source_pilot_idx
  on app.private_live_telebirr_source_recoveries (source_pilot_revision_id, authorized_at);
create index private_live_tbirr_source_recovery_target_pilot_idx
  on app.private_live_telebirr_source_recoveries (target_pilot_revision_id, authorized_at);

create trigger private_live_tbirr_source_recoveries_immutable
before update or delete on app.private_live_telebirr_source_recoveries
for each row execute function app.reject_private_live_telebirr_lineage_mutation();

create trigger private_live_tbirr_source_recoveries_no_truncate
before truncate on app.private_live_telebirr_source_recoveries
for each statement execute function app.reject_private_live_telebirr_lineage_truncate();

alter table app.private_live_telebirr_source_recoveries enable row level security;
alter table app.private_live_telebirr_source_recoveries force row level security;

create function app.private_live_telebirr_source_recovery_digest(
...
create function app.recover_private_live_telebirr_source_to_shadow(
  p_terminal_live_verification_job_id uuid,
  p_source_pilot_revision_id uuid,
  p_target_pilot_revision_id uuid,
  p_replacement_shadow_proof_request_id uuid,
...
create function app.private_live_telebirr_source_recovery_is_valid(
  p_replacement_shadow_proof_request_id uuid,
  p_recovery_request_key uuid
)
returns boolean
...
begin
  ...
  return recovery.recovery_request_key is not null
    ...
    and recovery.recovery_expires_at <= source_proof.submitted_at + interval '24 hours'
...
comment on function app.recover_private_live_telebirr_source_to_shadow(
  ...
  'Postgres-only creation of one five-minute advisory shadow request for the exact reviewed terminal live source_unavailable lineage. Original live rows remain append-only and no claim, reservation, settlement, execution job, or money movement is created.';
