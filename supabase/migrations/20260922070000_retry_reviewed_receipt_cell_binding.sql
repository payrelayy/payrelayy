-- Add one reusable append-only no-money receipt-cell-binding retry for the latest reviewed receipt.
-- The source is the immutable receipt-alias child and is eligible only when the authenticated
...
do $receipt_cell_retry_preflight$
begin
  if pg_catalog.to_regclass(
       'app.private_telebirr_shadow_receipt_alias_retries'
     ) is null
    or pg_catalog.to_regprocedure(
         'app.private_telebirr_shadow_receipt_alias_retry_is_valid(uuid,uuid)'
       ) is null
    or pg_catalog.to_regprocedure(
    ...
    raise exception 'The receipt-cell-binding retry prerequisites do not match.';
  end if;
end;
$receipt_cell_retry_preflight$;

create table app.private_telebirr_shadow_receipt_cell_binding_retries (
  retry_request_key uuid primary key,
  retry_request_digest text not null unique
    check (retry_request_digest ~ '^sha256:[0-9a-f]{64}$'),
  source_alias_retry_request_key uuid not null unique
    references app.private_telebirr_shadow_receipt_alias_retries (
      retry_request_key
    ) on delete restrict,
  source_alias_retry_request_digest text not null unique
    check (source_alias_retry_request_digest ~ '^sha256:[0-9a-f]{64}$'),
  ...
  source_receipt_cell_binding_review_count integer not null
    check (source_receipt_cell_binding_review_count = source_attempt_count),
  ...
  reason_code text not null check (
    reason_code = 'reviewed_receipt_cell_binding_retry_no_credit'
  ),
  created_at timestamptz not null default pg_catalog.clock_timestamp(),
  constraint private_tbirr_shadow_cell_retry_key_v4_check check (
    retry_request_key::text
      ~ '^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
  ),
  constraint private_tbirr_shadow_cell_retry_window_check check (
    retry_expires_at = authorized_at + interval '12 hours'
  ),
  constraint private_tbirr_shadow_cell_retry_source_job_fkey
    ...
    on delete restrict,
  constraint private_tbirr_shadow_cell_retry_replacement_job_fkey
    foreign key (
...
create function app.private_telebirr_shadow_receipt_cell_binding_retry_digest(
  p_retry_request_key uuid,
  p_source_alias_retry_request_key uuid,
  p_source_alias_retry_request_digest text,
  ...
  p_source_receipt_cell_binding_review_count integer,
...
begin
  if p_retry_request_key is null
    or p_source_alias_retry_request_key is null
    or p_source_alias_retry_request_digest !~ '^sha256:[0-9a-f]{64}$'
    ...
    or p_source_receipt_cell_binding_review_count is distinct from p_source_attempt_count
    ...
    or p_reason_code is distinct from
       'reviewed_receipt_cell_binding_retry_no_credit' then
    raise exception 'The receipt-cell-binding retry digest input is invalid.';
  end if;

  return app.private_live_deposit_pilot_sha256(
    'fetanagent:telebirr:reviewed-receipt-cell-binding-retry:v1'
      || '|request_key=' || p_retry_request_key::text
      || '|source_alias_key=' || p_source_alias_retry_request_key::text
      || '|source_alias_digest=' || p_source_alias_retry_request_digest
      ...
      || '|source_receipt_cell_binding_reviews='
      || p_source_receipt_cell_binding_review_count::text
...
create function app.private_telebirr_shadow_receipt_cell_binding_retry_child_is_valid(
...
begin
  ...
  return retry.retry_request_key is not null
    ...
    and app.private_telebirr_shadow_receipt_alias_retry_is_valid(
          source_proof.id,
          source_alias.retry_request_key
        )
    ...
    and app.private_telebirr_shadow_layout_evidence_history_digest(source_proof.id) =
        retry.source_evidence_history_digest
    and receipt_cell_binding_review_count = retry.source_receipt_cell_binding_review_count
    and outcome_count = retry.source_outcome_count
    and pilot.id = source_proof.pilot_revision_id
    and pilot.id = source_alias.pilot_revision_id
    and profile.id = source_proof.receiver_profile_id
    and profile.id = source_alias.receiver_profile_id
...
create function app.private_telebirr_shadow_receipt_cell_binding_retry_is_valid(
  p_replacement_shadow_proof_request_id uuid,
  p_retry_request_key uuid
)
returns boolean
...
begin
  ...
  return app.private_telebirr_shadow_receipt_cell_binding_retry_child_is_valid(
    replacement,
    p_retry_request_key
  );
exception
...
-- The new lineage keeps its own validator. Existing receipt-layout and receipt-alias
...
-- reviewed receipt-cell-binding path ahead of its historical receipt-layout path.
do $bind_receipt_cell_retry_insert_guard$
declare
  ...
  expected_source_sha256 constant text :=
    '06e317b6a51c1b92e33509d547a6e1f7c2a65f23abf38e8ed2bf2d6229799c2b';
  ...
  new_fragment constant text :=
    'begin' || pg_catalog.chr(10)
    || '  if pg_catalog.current_setting(''app.private_telebirr_shadow_receipt_cell_binding_retry'', true)'
    || pg_catalog.chr(10)
    || '       ~ ''^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'' then'
    || pg_catalog.chr(10)
    || '    if app.private_telebirr_shadow_receipt_cell_binding_retry_child_is_valid('
    || pg_catalog.chr(10)
    || '      new,' || pg_catalog.chr(10)
    || '      pg_catalog.current_setting(''app.private_telebirr_shadow_receipt_cell_binding_retry'', true)::uuid'
    || pg_catalog.chr(10)
    || '    ) then' || pg_catalog.chr(10)
    || '      return new;' || pg_catalog.chr(10)
    || '    end if;' || pg_catalog.chr(10)
    || '    raise exception ''The receipt-cell-binding retry child is invalid.'';'
    || pg_catalog.chr(10)
...
begin
  ...
  if not exists (
    ...
    raise exception 'The receipt-cell-binding insert-guard rewrite changed its authority.';
  end if;
end;
$bind_receipt_cell_retry_insert_guard$;
...
do $bind_receipt_cell_retry_deadline$
declare
  routine_oid oid := pg_catalog.to_regprocedure(
    'app.private_telebirr_shadow_source_binding_layout_review_deadline(uuid)'
  );
  expected_source_sha256 constant text :=
    'c51d2ae10ba6240a0b11c6a77b2a6ad2e6a783e832f2e8290bfc4bb3a567a83c';
  ...
  old_begin constant text :=
    'begin' || pg_catalog.chr(10)
    || '  select candidate.* into alias_retry';
  new_begin constant text :=
    'begin' || pg_catalog.chr(10)
    || '  select candidate.* into cell_retry' || pg_catalog.chr(10)
    || '    from app.private_telebirr_shadow_receipt_cell_binding_retries candidate'
    || pg_catalog.chr(10)
    || '   where candidate.replacement_shadow_proof_request_id = p_shadow_proof_request_id;'
    || pg_catalog.chr(10) || pg_catalog.chr(10)
    || '  if cell_retry.retry_request_key is not null' || pg_catalog.chr(10)
    || '    and app.private_telebirr_shadow_receipt_cell_binding_retry_is_valid('
    || pg_catalog.chr(10)
    || '      p_shadow_proof_request_id,' || pg_catalog.chr(10)
    || '      cell_retry.retry_request_key' || pg_catalog.chr(10)
    || '    ) then' || pg_catalog.chr(10)
    || '    return cell_retry.retry_expires_at;' || pg_catalog.chr(10)
    || '  end if;' || pg_catalog.chr(10) || pg_catalog.chr(10)
    || '  select candidate.* into alias_retry';
  original_definition text;
  original_source text;
...
begin
  ...
  if not exists (
    ...
    raise exception 'The receipt-cell-binding deadline rewrite changed its authority.';
  end if;
end;
$bind_receipt_cell_retry_deadline$;

create function app.retry_reviewed_private_telebirr_receipt_cell_binding(
  p_reviewed_main_commit_sha text,
  p_reason_code text
)
returns table (
  created boolean,
  receipt_cell_binding_retry_count integer,
  shadow_request_count integer,
  source_shadow_attempt_count integer,
  source_staged_evidence_count integer,
  source_receipt_cell_binding_review_count integer,
...
as $$
declare
  existing_retry app.private_telebirr_shadow_receipt_cell_binding_retries%rowtype;
  source_alias app.private_telebirr_shadow_receipt_alias_retries%rowtype;
  source_proof app.private_telebirr_shadow_proof_requests%rowtype;
  source_outcome app.private_telebirr_shadow_verification_outcomes%rowtype;
...
begin
  if session_user <> 'postgres'
    ...
    raise exception 'The receipt-cell-binding retry request is invalid.';
  end if;
  ...
  perform pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(
      'fetanagent:telebirr:reviewed-receipt-cell-binding-retry',
      0
    )
  );
  ...
  if exists (
    ...
    raise exception 'The receipt-cell-binding retry requires every execution login and session absent.';
  end if;

  select pg_catalog.count(*)::integer into existing_retry_count
    from app.private_telebirr_shadow_receipt_cell_binding_retries;
  ...
  if existing_retry_count <> 0 then
    ...
    and app.private_telebirr_shadow_receipt_cell_binding_retry_is_valid(
      ...
      select false, 1, 1,
             ...
             existing_retry.source_receipt_cell_binding_review_count,
    ...
    raise exception 'The receipt-cell-binding retry already exists or conflicts.';
  end if;

  select pg_catalog.count(*)::integer into candidate_count
    from app.private_telebirr_shadow_receipt_alias_retries alias_retry
    join app.private_telebirr_shadow_proof_requests proof
      on proof.id = alias_retry.replacement_shadow_proof_request_id
     and proof.verification_job_id = alias_retry.replacement_shadow_verification_job_id
    join app.private_telebirr_shadow_verification_outcomes outcome
      on outcome.shadow_proof_request_id = proof.id
    left join app.private_telebirr_shadow_receipt_cell_binding_retries cell_retry
      on cell_retry.source_alias_retry_request_key = alias_retry.retry_request_key
   where cell_retry.retry_request_key is null
     and alias_retry.reason_code =
     ...
     and exists (
      ...
      and app.private_telebirr_shadow_receipt_alias_retry_is_valid(
           proof.id,
           alias_retry.retry_request_key
         );

  if candidate_count <> 1 then
    raise exception 'Exactly one reviewed receipt-cell-binding source is required.';
  end if;

  select alias_retry.* into source_alias
    from app.private_telebirr_shadow_receipt_alias_retries alias_retry
    join app.private_telebirr_shadow_proof_requests proof
      on proof.id = alias_retry.replacement_shadow_proof_request_id
     and proof.verification_job_id = alias_retry.replacement_shadow_verification_job_id
   ...
   for share of alias_retry, proof, outcome;

  select proof.* into source_proof
    from app.private_telebirr_shadow_proof_requests proof
   where proof.id = source_alias.replacement_shadow_proof_request_id
     and proof.verification_job_id = source_alias.replacement_shadow_verification_job_id
   for key share;
  select outcome.* into source_outcome
    from app.private_telebirr_shadow_verification_outcomes outcome
   where outcome.shadow_proof_request_id = source_proof.id
  ...
  select pg_catalog.count(*)::integer into v_ready_enrollment_count
   ...
   where enrollment.pilot_revision_id = target_pilot.id
     ...
     and heartbeat.app_version = '0.5.8-evidence-only'
  ...
  select pg_catalog.count(*)::integer into source_staged
    ...
    join app.private_telebirr_shadow_verification_attempts attempt
      on attempt.id = staged.verification_attempt_id
   where attempt.shadow_proof_request_id = source_proof.id;
  select pg_catalog.count(*)::integer into source_alias_reviews
    from app.private_telebirr_shadow_device_evidence_staging staged
    join app.private_telebirr_shadow_verification_attempts attempt
      on attempt.id = staged.verification_attempt_id
   where attempt.shadow_proof_request_id = source_proof.id
  ...
  if locked_switch_count <> 7
    or source_alias.retry_request_key is null
    ...
    or source_alias_reviews <> source_attempts
    ...
    or source_alias.pilot_revision_id <> target_pilot.id
    or source_alias.receiver_profile_id <> target_profile.id
    ...
    or not app.private_telebirr_shadow_receipt_alias_retry_is_valid(
         source_proof.id,
         source_alias.retry_request_key
       )
  ...
  retry_digest := app.private_telebirr_shadow_receipt_cell_binding_retry_digest(
    v_retry_request_key,
    source_alias.retry_request_key,
    source_alias.retry_request_digest,
    source_proof.id,
    source_proof.verification_job_id,
    source_outcome.id,
  ...
  insert into app.private_telebirr_shadow_receipt_cell_binding_retries (
    retry_request_key, retry_request_digest,
    source_alias_retry_request_key, source_alias_retry_request_digest,
    ...
    source_receipt_cell_binding_review_count, source_outcome_count,
    authorized_at, retry_expires_at, reviewed_main_commit_sha, reason_code
  ) values (
    v_retry_request_key, retry_digest,
    source_alias.retry_request_key, source_alias.retry_request_digest,
    ...
    source_alias_reviews, source_outcomes,
    v_authorized_at, v_retry_until, p_reviewed_main_commit_sha, p_reason_code
  );

  perform pg_catalog.set_config(
    'app.private_telebirr_shadow_receipt_cell_binding_retry',
  ...
  if not app.private_telebirr_shadow_receipt_cell_binding_retry_is_valid(
    inserted_proof.id,
    v_retry_request_key
  ) then
    raise exception 'The receipt-cell-binding retry did not validate exactly.';
  end if;
  ...
  insert into app.audit_events (
    ...
    'worker',
    'reviewed-receipt-cell-binding-retry',
    'deposit.telebirr_receipt_cell_binding_retried',
    'private_live_deposit_pilot',
    target_pilot.id,
    pg_catalog.jsonb_build_object(
      ...
      'source_receipt_cell_binding_review_count', source_alias_reviews,
  ...
  return query
  select true, 1, 1,
         source_attempts, source_staged, source_alias_reviews, source_outcomes,
...
create trigger private_tbirr_shadow_cell_binding_retries_immutable
before update or delete on app.private_telebirr_shadow_receipt_cell_binding_retries
for each row execute function app.reject_private_live_telebirr_lineage_mutation();

create trigger private_tbirr_shadow_cell_binding_retries_no_truncate
before truncate on app.private_telebirr_shadow_receipt_cell_binding_retries
for each statement execute function app.reject_private_live_telebirr_lineage_truncate();

alter table app.private_telebirr_shadow_receipt_cell_binding_retries enable row level security;
alter table app.private_telebirr_shadow_receipt_cell_binding_retries force row level security;
alter table app.private_telebirr_shadow_receipt_cell_binding_retries owner to postgres;

alter function app.private_telebirr_shadow_receipt_cell_binding_retry_digest(
...
alter function app.private_telebirr_shadow_receipt_cell_binding_retry_child_is_valid(
  app.private_telebirr_shadow_proof_requests, uuid
) owner to postgres;
alter function app.private_telebirr_shadow_receipt_cell_binding_retry_is_valid(uuid, uuid)
  owner to postgres;
alter function app.retry_reviewed_private_telebirr_receipt_cell_binding(text, text)
  owner to postgres;

revoke all on table app.private_telebirr_shadow_receipt_cell_binding_retries
  from public, anon, authenticated, service_role;
revoke all on function app.private_telebirr_shadow_receipt_cell_binding_retry_digest(
  uuid, uuid, text, uuid, uuid, uuid, uuid, uuid, uuid, uuid, uuid, uuid,
  integer, text, integer, text, integer, integer, timestamptz, timestamptz, text, text
) from public, anon, authenticated, service_role;
revoke all on function app.private_telebirr_shadow_receipt_cell_binding_retry_child_is_valid(
  app.private_telebirr_shadow_proof_requests, uuid
) from public, anon, authenticated, service_role;
revoke all on function app.private_telebirr_shadow_receipt_cell_binding_retry_is_valid(uuid, uuid)
  from public, anon, authenticated, service_role;
revoke all on function app.retry_reviewed_private_telebirr_receipt_cell_binding(text, text)
  from public, anon, authenticated, service_role;

comment on table app.private_telebirr_shadow_receipt_cell_binding_retries is
  'Immutable one-use no-money lineage for re-evaluating one authenticated signed receipt after a bounded official receipt-cell binding correction.';
comment on function app.private_telebirr_shadow_receipt_cell_binding_retry_is_valid(uuid, uuid) is
  'Validates the exact receipt-cell-binding retry child, its immutable review-only source, current paired verifier, and complete no-money boundary.';
comment on function app.retry_reviewed_private_telebirr_receipt_cell_binding(text, text) is
  'Creates exactly one fixed twelve-hour append-only no-money child for the reviewed official receipt-cell binding correction.';
