-- Add one reusable append-only no-money receipt-cell-opening retry for the latest reviewed receipt.
-- The source is the immutable receipt-cell-binding child and is eligible only when the authenticated
...
begin
  if pg_catalog.to_regclass(
    ...
    raise exception 'The receipt-cell-opening retry prerequisites do not match.';
  end if;
end;
$receipt_cell_retry_preflight$;

create table app.private_telebirr_shadow_receipt_cell_opening_retries (
  retry_request_key uuid primary key,
  retry_request_digest text not null unique
    check (retry_request_digest ~ '^sha256:[0-9a-f]{64}$'),
  source_cell_binding_retry_request_key uuid not null unique
    references app.private_telebirr_shadow_receipt_cell_binding_retries (
      retry_request_key
    ) on delete restrict,
  source_cell_binding_retry_request_digest text not null unique
    check (source_cell_binding_retry_request_digest ~ '^sha256:[0-9a-f]{64}$'),
  ...
  source_receipt_cell_opening_review_count integer not null
    check (source_receipt_cell_opening_review_count = source_attempt_count),
  ...
  reason_code text not null check (
    reason_code = 'reviewed_receipt_cell_opening_retry_no_credit'
  ),
...
create function app.private_telebirr_shadow_receipt_cell_opening_retry_digest(
  p_retry_request_key uuid,
  p_source_cell_binding_retry_request_key uuid,
  p_source_cell_binding_retry_request_digest text,
  ...
  p_source_receipt_cell_opening_review_count integer,
...
begin
  if p_retry_request_key is null
    or p_source_cell_binding_retry_request_key is null
    or p_source_cell_binding_retry_request_digest !~ '^sha256:[0-9a-f]{64}$'
    ...
    or p_source_receipt_cell_opening_review_count is distinct from p_source_attempt_count
    ...
    or p_reason_code is distinct from
       'reviewed_receipt_cell_opening_retry_no_credit' then
    raise exception 'The receipt-cell-opening retry digest input is invalid.';
  end if;

  return app.private_live_deposit_pilot_sha256(
    'fetanagent:telebirr:reviewed-receipt-cell-opening-retry:v1'
      || '|request_key=' || p_retry_request_key::text
      || '|source_cell_binding_key=' || p_source_cell_binding_retry_request_key::text
      || '|source_cell_binding_digest=' || p_source_cell_binding_retry_request_digest
      ...
      || '|source_receipt_cell_opening_reviews='
      || p_source_receipt_cell_opening_review_count::text
...
create function app.private_telebirr_shadow_receipt_cell_opening_retry_child_is_valid(
...
as $$
declare
  retry app.private_telebirr_shadow_receipt_cell_opening_retries%rowtype;
  source_cell_binding app.private_telebirr_shadow_receipt_cell_binding_retries%rowtype;
  ...
  receipt_cell_opening_review_count integer;
...
begin
  ...
  select candidate.* into source_cell_binding
    from app.private_telebirr_shadow_receipt_cell_binding_retries candidate
   where candidate.retry_request_key = retry.source_cell_binding_retry_request_key;
  ...
  select pg_catalog.count(*)::integer into staged_count
   ...
   where attempt.shadow_proof_request_id = source_proof.id;
  select pg_catalog.count(*)::integer into receipt_cell_opening_review_count
    from app.private_telebirr_shadow_device_evidence_staging staged
    join app.private_telebirr_shadow_verification_attempts attempt
      on attempt.id = staged.verification_attempt_id
   where attempt.shadow_proof_request_id = source_proof.id
  ...
  expected_digest := app.private_telebirr_shadow_receipt_cell_opening_retry_digest(
  ...
  return retry.retry_request_key is not null
    ...
    and retry.source_cell_binding_retry_request_digest = source_cell_binding.retry_request_digest
    ...
    and app.private_telebirr_shadow_layout_evidence_history_digest(source_proof.id) =
        retry.source_evidence_history_digest
    and receipt_cell_opening_review_count = retry.source_receipt_cell_opening_review_count
    ...
    and retry.reason_code = 'reviewed_receipt_cell_opening_retry_no_credit'
...
create function app.private_telebirr_shadow_receipt_cell_opening_retry_is_valid(
  p_replacement_shadow_proof_request_id uuid,
  p_retry_request_key uuid
)
returns boolean
...
begin
  ...
  return app.private_telebirr_shadow_receipt_cell_opening_retry_child_is_valid(
    replacement,
    p_retry_request_key
  );
exception
...
-- reviewed receipt-cell-opening path ahead of its historical receipt-layout path.
...
declare
  ...
  expected_source_sha256 constant text :=
    '45ef66f5159a6eb6bd277bab0315a5afc74a4d0009d5f04e0d4120ac02f51fa2';
  ...
  new_fragment constant text :=
    ...
    || pg_catalog.chr(10)
    || '    if app.private_telebirr_shadow_receipt_cell_opening_retry_child_is_valid('
    || pg_catalog.chr(10)
    || '      new,' || pg_catalog.chr(10)
    || '      pg_catalog.current_setting(''app.private_telebirr_shadow_receipt_cell_opening_retry'', true)::uuid'
    || pg_catalog.chr(10)
    || '    ) then' || pg_catalog.chr(10)
    || '      return new;' || pg_catalog.chr(10)
    || '    end if;' || pg_catalog.chr(10)
    || '    raise exception ''The receipt-cell-opening retry child is invalid.'';'
    || pg_catalog.chr(10)
...
begin
  ...
  if not exists (
    ...
    raise exception 'The receipt-cell-opening insert-guard rewrite changed its authority.';
  end if;
end;
...
declare
  ...
  expected_source_sha256 constant text :=
    'fbd7f45ed83ba42f0bb5e806e3282df3c7caef5dbd4e981410e20b13955c6401';
  ...
  old_begin constant text :=
    'begin' || pg_catalog.chr(10)
    || '  select candidate.* into cell_retry';
  new_begin constant text :=
    'begin' || pg_catalog.chr(10)
    || '  select candidate.* into opening_retry' || pg_catalog.chr(10)
    || '    from app.private_telebirr_shadow_receipt_cell_opening_retries candidate'
    || pg_catalog.chr(10)
    || '   where candidate.replacement_shadow_proof_request_id = p_shadow_proof_request_id;'
    || pg_catalog.chr(10) || pg_catalog.chr(10)
    || '  if opening_retry.retry_request_key is not null' || pg_catalog.chr(10)
    || '    and app.private_telebirr_shadow_receipt_cell_opening_retry_is_valid('
    || pg_catalog.chr(10)
    || '      p_shadow_proof_request_id,' || pg_catalog.chr(10)
    || '      opening_retry.retry_request_key' || pg_catalog.chr(10)
    || '    ) then' || pg_catalog.chr(10)
    || '    return opening_retry.retry_expires_at;' || pg_catalog.chr(10)
    || '  end if;' || pg_catalog.chr(10) || pg_catalog.chr(10)
    || '  select candidate.* into cell_retry';
  original_definition text;
  original_source text;
...
begin
  ...
  if not exists (
    ...
    raise exception 'The receipt-cell-opening deadline rewrite changed its authority.';
  end if;
end;
$bind_receipt_cell_retry_deadline$;

create function app.retry_reviewed_private_telebirr_receipt_cell_opening(
  p_reviewed_main_commit_sha text,
  p_reason_code text
)
returns table (
  created boolean,
  receipt_cell_opening_retry_count integer,
  shadow_request_count integer,
  source_shadow_attempt_count integer,
  source_staged_evidence_count integer,
  source_receipt_cell_opening_review_count integer,
...
as $$
declare
  existing_retry app.private_telebirr_shadow_receipt_cell_opening_retries%rowtype;
  source_cell_binding app.private_telebirr_shadow_receipt_cell_binding_retries%rowtype;
  source_proof app.private_telebirr_shadow_proof_requests%rowtype;
  source_outcome app.private_telebirr_shadow_verification_outcomes%rowtype;
  ...
  source_cell_binding_reviews integer;
...
begin
  if session_user <> 'postgres'
    ...
    raise exception 'The receipt-cell-opening retry request is invalid.';
  end if;
  ...
  perform pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended(
      'fetanagent:telebirr:reviewed-receipt-cell-opening-retry',
      0
    )
  );
  ...
  if exists (
    ...
    raise exception 'The receipt-cell-opening retry requires every execution login and session absent.';
  end if;
  ...
  if existing_retry_count <> 0 then
    ...
    and app.private_telebirr_shadow_receipt_cell_opening_retry_is_valid(
      ...
      select false, 1, 1,
             ...
             existing_retry.source_receipt_cell_opening_review_count,
    ...
    raise exception 'The receipt-cell-opening retry already exists or conflicts.';
  end if;
  ...
  select pg_catalog.count(*)::integer into candidate_count
    ...
    left join app.private_telebirr_shadow_receipt_cell_opening_retries cell_retry
      on cell_retry.source_cell_binding_retry_request_key = cell_binding_retry.retry_request_key
  ...
  if candidate_count <> 1 then
    raise exception 'Exactly one reviewed receipt-cell-opening source is required.';
  end if;

  select cell_binding_retry.* into source_cell_binding
    from app.private_telebirr_shadow_receipt_cell_binding_retries cell_binding_retry
    join app.private_telebirr_shadow_proof_requests proof
      on proof.id = cell_binding_retry.replacement_shadow_proof_request_id
     and proof.verification_job_id = cell_binding_retry.replacement_shadow_verification_job_id
   ...
   for share of cell_binding_retry, proof, outcome;

  select proof.* into source_proof
    from app.private_telebirr_shadow_proof_requests proof
   where proof.id = source_cell_binding.replacement_shadow_proof_request_id
     and proof.verification_job_id = source_cell_binding.replacement_shadow_verification_job_id
   for key share;
  select outcome.* into source_outcome
    from app.private_telebirr_shadow_verification_outcomes outcome
   where outcome.shadow_proof_request_id = source_proof.id
  ...
  select pg_catalog.count(*)::integer into source_staged
    ...
    join app.private_telebirr_shadow_verification_attempts attempt
      on attempt.id = staged.verification_attempt_id
   where attempt.shadow_proof_request_id = source_proof.id;
  select pg_catalog.count(*)::integer into source_cell_binding_reviews
    from app.private_telebirr_shadow_device_evidence_staging staged
    join app.private_telebirr_shadow_verification_attempts attempt
      on attempt.id = staged.verification_attempt_id
   where attempt.shadow_proof_request_id = source_proof.id
  ...
  if locked_switch_count <> 7
    or source_cell_binding.retry_request_key is null
    ...
    or source_cell_binding_reviews <> source_attempts
    ...
    or not app.private_telebirr_shadow_receipt_cell_binding_retry_is_valid(
         source_proof.id,
         source_cell_binding.retry_request_key
       )
  ...
  retry_digest := app.private_telebirr_shadow_receipt_cell_opening_retry_digest(
    v_retry_request_key,
    source_cell_binding.retry_request_key,
  ...
  insert into app.private_telebirr_shadow_receipt_cell_opening_retries (
    retry_request_key, retry_request_digest,
    source_cell_binding_retry_request_key, source_cell_binding_retry_request_digest,
    ...
    source_receipt_cell_opening_review_count, source_outcome_count,
    authorized_at, retry_expires_at, reviewed_main_commit_sha, reason_code
  ) values (
    v_retry_request_key, retry_digest,
    source_cell_binding.retry_request_key, source_cell_binding.retry_request_digest,
    ...
    source_cell_binding_reviews, source_outcomes,
    v_authorized_at, v_retry_until, p_reviewed_main_commit_sha, p_reason_code
  );

  perform pg_catalog.set_config(
    'app.private_telebirr_shadow_receipt_cell_opening_retry',
  ...
  if not app.private_telebirr_shadow_receipt_cell_opening_retry_is_valid(
    inserted_proof.id,
    v_retry_request_key
  ) then
    raise exception 'The receipt-cell-opening retry did not validate exactly.';
  end if;
  ...
  insert into app.audit_events (
    ...
    'worker',
    'reviewed-receipt-cell-opening-retry',
    'deposit.telebirr_receipt_cell_opening_retried',
    'private_live_deposit_pilot',
    target_pilot.id,
    pg_catalog.jsonb_build_object(
      ...
      'source_receipt_cell_opening_review_count', source_cell_binding_reviews,
  ...
  return query
  select true, 1, 1,
         source_attempts, source_staged, source_cell_binding_reviews, source_outcomes,
...
create trigger private_tbirr_shadow_cell_opening_retries_immutable
before update or delete on app.private_telebirr_shadow_receipt_cell_opening_retries
for each row execute function app.reject_private_live_telebirr_lineage_mutation();

create trigger private_tbirr_shadow_cell_opening_retries_no_truncate
before truncate on app.private_telebirr_shadow_receipt_cell_opening_retries
for each statement execute function app.reject_private_live_telebirr_lineage_truncate();

alter table app.private_telebirr_shadow_receipt_cell_opening_retries enable row level security;
alter table app.private_telebirr_shadow_receipt_cell_opening_retries force row level security;
alter table app.private_telebirr_shadow_receipt_cell_opening_retries owner to postgres;

alter function app.private_telebirr_shadow_receipt_cell_opening_retry_digest(
...
alter function app.private_telebirr_shadow_receipt_cell_opening_retry_child_is_valid(
  app.private_telebirr_shadow_proof_requests, uuid
) owner to postgres;
alter function app.private_telebirr_shadow_receipt_cell_opening_retry_is_valid(uuid, uuid)
  owner to postgres;
alter function app.retry_reviewed_private_telebirr_receipt_cell_opening(text, text)
  owner to postgres;

revoke all on table app.private_telebirr_shadow_receipt_cell_opening_retries
  from public, anon, authenticated, service_role;
revoke all on function app.private_telebirr_shadow_receipt_cell_opening_retry_digest(
  uuid, uuid, text, uuid, uuid, uuid, uuid, uuid, uuid, uuid, uuid, uuid,
  integer, text, integer, text, integer, integer, timestamptz, timestamptz, text, text
) from public, anon, authenticated, service_role;
revoke all on function app.private_telebirr_shadow_receipt_cell_opening_retry_child_is_valid(
  app.private_telebirr_shadow_proof_requests, uuid
) from public, anon, authenticated, service_role;
revoke all on function app.private_telebirr_shadow_receipt_cell_opening_retry_is_valid(uuid, uuid)
  from public, anon, authenticated, service_role;
revoke all on function app.retry_reviewed_private_telebirr_receipt_cell_opening(text, text)
  from public, anon, authenticated, service_role;

comment on table app.private_telebirr_shadow_receipt_cell_opening_retries is
  'Immutable one-use no-money lineage for re-evaluating one authenticated signed receipt after a bounded official receipt-cell opening-tag correction.';
comment on function app.private_telebirr_shadow_receipt_cell_opening_retry_is_valid(uuid, uuid) is
  'Validates the exact receipt-cell-opening retry child, its immutable review-only source, current paired verifier, and complete no-money boundary.';
comment on function app.retry_reviewed_private_telebirr_receipt_cell_opening(text, text) is
  'Creates exactly one fixed twelve-hour append-only no-money child for the reviewed official receipt-cell opening-tag correction.';
