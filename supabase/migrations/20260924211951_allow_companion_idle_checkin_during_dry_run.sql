-- Keep an enrolled Windows companion's authenticated presence visible while an
-- inert, fixed dry-run TeleBirr pilot is armed. This function returns only the
-- empty idle response in that state: no Player IDs or lookup assignment can be
-- claimed or delivered. Every non-idle lookup still uses the original boundary.
...
declare
  ...
  switches_safe boolean;
  pilot_switch_mode text;
  pilot_switch_settings jsonb;
  open_pilot_count integer;
...
begin
  ...
  -- The pilot switch may be dry_run only when it points to the one exact,
  -- financially inactive armed revision. Every other previously checked
  -- switch remains disabled. A live pilot or a draft/expired pilot fails closed.
  select count(*)::integer,
         coalesce(bool_and(
           feature.mode = 'disabled'
           or (feature.feature_key = 'private_live_deposit_pilot'
               and feature.mode = 'dry_run')
         ), false)
    into switch_count, switches_safe
  ...
  if switch_count <> 5 or not switches_safe then
    raise exception 'The companion idle poll requires every money switch to be disabled.';
  end if;
  select feature.mode::text, feature.settings
    into pilot_switch_mode, pilot_switch_settings
    from app.feature_switches feature
   where feature.feature_key = 'private_live_deposit_pilot';

  select count(*)::integer
    into open_pilot_count
    from app.private_live_deposit_pilot_revisions open_pilot
   where open_pilot.status in ('draft', 'armed');
  if pilot_switch_mode = 'disabled' then
    if open_pilot_count <> 0 then
      raise exception 'The companion idle poll requires no open private live-money pilot.';
    end if;
  elsif pilot_switch_mode = 'dry_run' then
    select open_pilot.*
      into pilot
      from app.private_live_deposit_pilot_revisions open_pilot
     where open_pilot.status = 'armed';
    if open_pilot_count <> 1
      or pilot.id is null
      or pilot.active_from > now_at
      or pilot.expires_at <= now_at
      or pilot_switch_settings is distinct from pg_catalog.jsonb_build_object(
        'contract_version', 1,
      ...
      raise exception 'The companion idle poll requires one unreserved armed dry-run pilot.';
    end if;
  else
    raise exception 'The companion idle poll requires every money switch to be disabled.';
  end if;
  ...
  -- An armed dry-run pilot accepts only presence telemetry. A pre-existing
  -- outstanding lookup is never claimed or returned across this boundary.
  if pilot_switch_mode = 'dry_run' and exists (
    select 1 from app.agent_platform_companion_lookup_assignments assignment
    ...
    raise exception 'The companion idle poll cannot deliver an assignment during a dry-run pilot.';
  end if;
  ...
  if pilot_switch_mode = 'dry_run' then
  ...
  -- The original full profile/eligibility boundary remains mandatory before
  -- any non-empty Find-only assignment, including a previously signed one.
